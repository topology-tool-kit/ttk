#include                  <vtkScalarFieldNormalizer.h>

vtkStandardNewMacro(vtkScalarFieldNormalizer)

vtkScalarFieldNormalizer::vtkScalarFieldNormalizer(){
...
bool vtkScalarFieldNormalizer::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkScalarFieldNormalizer::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkScalarFieldNormalizer] " << progress*100 
      << "% processed...." << endl;
...
int vtkScalarFieldNormalizer::normalize(
  vtkDataArray *input, vtkDataArray *output) const{
...
int vtkScalarFieldNormalizer::doIt(vtkDataSet *input, vtkDataSet *output){

  // test
//   vtkTriangulation myTriangulation;
...
//   }
  ...
  if(!outputScalarField_){
    switch(inputScalarField->GetDataType()){
      ...
      stringstream msg;
      msg << "[vtkScalarFieldNormalizer] Unsupported data type :(" << endl;
...
int vtkScalarFieldNormalizer::RequestData(vtkInformation *request, 
  ...
  {
    stringstream msg;
    msg << "[vtkScalarFieldNormalizer] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
