...
vtkStandardNewMacro(vtkTextureMapFromField)

vtkTextureMapFromField::vtkTextureMapFromField(){
...
bool vtkTextureMapFromField::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkTextureMapFromField::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkTextureMapFromField] " << progress*100 
      << "% processed...." << endl;
...
int vtkTextureMapFromField::doIt(vtkDataSet *input, vtkDataSet *output){
...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  {
    stringstream msg;
    msg << "[vtkTextureMapFromField] Texture map computed in " 
...
int vtkTextureMapFromField::RequestData(vtkInformation *request, 
  ...
  {
    stringstream msg;
    msg << "[vtkTextureMapFromField] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
