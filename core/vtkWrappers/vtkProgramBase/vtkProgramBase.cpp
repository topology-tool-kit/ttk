...
int vtkProgramBase::load(const vector<string> &inputPaths){
  ...
  for(int i = 0; i < (int) inputPaths.size(); i++){
    ...
    if(extension == "vti"){
      ...
      stringstream msg;
      msg << "[vtkProgramBase] Unkown input extension `" 
        << extension << "' :(" << endl;
