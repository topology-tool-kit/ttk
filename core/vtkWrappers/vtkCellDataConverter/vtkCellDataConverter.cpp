#include                  <vtkCellDataConverter.h>

vtkStandardNewMacro(vtkCellDataConverter)

vtkCellDataConverter::vtkCellDataConverter(){
}
...
bool vtkCellDataConverter::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkCellDataConverter::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkCellDataConverter] " << progress*100 
	<< "% processed...." << endl;
...
int vtkCellDataConverter::convert(vtkDataArray* inputData, vtkDataSet* output){
...
int vtkCellDataConverter::doIt(vtkDataSet *input, vtkDataSet *output){
...
int vtkCellDataConverter::RequestData(vtkInformation *request, 
  ...
  {
    stringstream msg;
    msg << "[vtkCellDataConverter] Memory usage: " << m.getElapsedUsage() 
	<< " MB." << endl;
