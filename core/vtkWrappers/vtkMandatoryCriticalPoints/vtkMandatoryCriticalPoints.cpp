...
vtkStandardNewMacro(vtkMandatoryCriticalPoints)

vtkMandatoryCriticalPoints::vtkMandatoryCriticalPoints(){
...
/////////////////////////////////////////////////////
/////////////////////////////////////////////////////
// void vtkMandatoryCriticalPoints::buildMandatorySplitTree(vtkUnstructuredGrid *outputSplitTree) {
//
//   // Clear the objects
//   if(mandatorySplitTreePoints_)
//     mandatorySplitTreePoints_->Delete();
...
//
//   cout << "[vtkMandatoryCriticalPoints] Number of points : ";
//   cout << mandatorySplitTree.getNumberOfPoints() << endl;
//   cout << "[vtkMandatoryCriticalPoints] Number of edges : ";
...
//     }
//   }
//   cout << "[vtkMandatoryCriticalPoints] rootGraphPointId = ";
...
int vtkMandatoryCriticalPoints::buildVtkTree(vtkUnstructuredGrid *outputTree, 
MandatoryCriticalPoints::TreeType treeType) {
  ...
  vtkPoints **mdtTreePoints =
    (treeType == MandatoryCriticalPoints::TreeType::JoinTree) ?
    &( mandatoryJoinTreePoints_ ) : &( mandatorySplitTreePoints_ );
  vector<vtkIdList *> *mdtTreeEdge =
    (treeType == MandatoryCriticalPoints::TreeType::JoinTree) ?
    &( mandatoryJoinTreeEdge_ ) : &( mandatorySplitTreeEdge_ );
  ...
  vtkIntArray **outputTreePointComponentId =
    (treeType == MandatoryCriticalPoints::TreeType::JoinTree) ?
    &( mdtJoinTreePointComponentId_ ) : &( mdtSplitTreePointComponentId_ );

  vtkIntArray **outputTreeEdgeSwitchable =
    (treeType == MandatoryCriticalPoints::TreeType::JoinTree) ?
    &( mdtJoinTreeEdgeSwitchable_ ) : &( mdtSplitTreeEdgeSwitchable_ );
...
// int vtkMandatoryCriticalPoints::buildMandatoryTree(vtkUnstructuredGrid 
...
bool vtkMandatoryCriticalPoints::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkMandatoryCriticalPoints::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkMandatoryCriticalPoints] " << progress*100
      << "% processed...." << endl;
...
int vtkMandatoryCriticalPoints::doIt(vector<vtkDataSet *> &inputs,
  vector<vtkDataSet *> &outputs){
//   vtkDataSet *input, vtkDataSet 
// *outputMinimum, vtkDataSet *outputJoinSaddle, vtkDataSet *outputSplitSaddle, 
// vtkDataSet *outputMaximum, vtkUnstructuredGrid *outputJoinTree, 
// vtkUnstructuredGrid *outputSplitTree){
  ...
  {
    stringstream msg;
    msg << "[vtkMandatoryCriticalPoints] Using `"
  ...
  if((triangulation_->isEmpty())
    ||(vtkTriangulation::hasChangedConnectivity(triangulation_, input, this))){
    Modified();
...

  {
    ...
    msg << "[vtkMandatoryCriticalPoints] Memory usage: " << memoryUsage_
        << " MB." << endl;
...
int vtkMandatoryCriticalPoints::FillInputPortInformation(int port, vtkInformation *info){
...
int vtkMandatoryCriticalPoints::FillOutputPortInformation(int port, vtkInformation *info){
