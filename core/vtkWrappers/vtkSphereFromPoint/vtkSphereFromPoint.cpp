#include                  <vtkSphereFromPoint.h>

vtkStandardNewMacro(vtkSphereFromPoint)

vtkSphereFromPoint::vtkSphereFromPoint(){
...
bool vtkSphereFromPoint::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkSphereFromPoint::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkSphereFromPoint] " << progress*100 
      << "% processed...." << endl;
...
int vtkSphereFromPoint::doIt(vtkDataSet *input, vtkPolyData *output){
...
#pragma omp parallel for num_threads(threadNumber_)
        ...
        if(array->GetNumberOfComponents() == 1){
          ...
          switch(array->GetDataType()){
            ...
            default:
              {
                stringstream msg;
                msg << "[vtkSphereFromPoint] Unsupported data type :(" << endl;
          ...
          stringstream msg;
          msg << "[vtkSphereFromPoint] Unsupported number of components :(" 
            << endl;
          dMsg(cerr, msg.str(), detailedInfoMsg);
        }
      }
  ...
  {
    stringstream msg;
    msg << "[vtkSphereFromPoint] Spheres computed in " 
...
int vtkSphereFromPoint::RequestData(vtkInformation *request, 
  ...
  {
    stringstream msg;
    msg << "[vtkSphereFromPoint] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
