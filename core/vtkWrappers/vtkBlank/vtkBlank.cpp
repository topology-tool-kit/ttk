...
vtkStandardNewMacro(vtkBlank)

int vtkBlank::doIt(vector<vtkDataSet *> &inputs, vector<vtkDataSet *> &outputs){
  ...
  if(!outputScalarField_){
    switch(inputScalarField->GetDataType()){
      ...
      stringstream msg;
      msg << "[vtkBlank] Unsupported data type :(" << endl;
  ...
  {
    stringstream msg;
    msg << "[vtkBlank] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
