...
vtkStandardNewMacro(vtkUncertainDataEstimator)

vtkUncertainDataEstimator::vtkUncertainDataEstimator(){
...
bool vtkUncertainDataEstimator::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkUncertainDataEstimator::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkUncertainDataEstimator] " << progress*100
      << "% processed...." << endl;
...
int vtkUncertainDataEstimator::doIt(vtkDataSet **input, vtkDataSet *outputBoundFields, vtkDataSet *outputProbability, vtkDataSet *outputMean, int numInputs){
  ...
  for(int i=0 ; i<numInputs ; i++){
    ...
    if(inputScalarField[i]->GetDataType() != inputScalarField[0]->GetDataType()){
      stringstream msg;
      msg << "[vtkUncertainDataEstimator] Inputs of different data types." << endl;
      dMsg(cerr, msg.str(), fatalMsg);
      return -3;
      }
    if(inputScalarField[i]->GetNumberOfTuples() != inputScalarField[0]->GetNumberOfTuples()){
      stringstream msg;
      msg << "[vtkUncertainDataEstimator] Inputs with different number of points." << endl;
      dMsg(cerr, msg.str(), fatalMsg);
      return -2;
      }
  ...
  if(!outputLowerBoundScalarField_
    ...
    switch(inputScalarField[0]->GetDataType()){
      ...
      stringstream msg;
      msg << "[vtkUncertainDataEstimator] Unsupported data type :(" << endl;
      dMsg(cerr, msg.str(), fatalMsg);
    }
    ...
    vtkTemplateMacro((
      {
        ...
        uncertainDataEstimator.setVertexNumber(outputBoundFields->GetNumberOfPoints());

        uncertainDataEstimator.setNumberOfInputs(numInputs);
        for(int i=0 ; i<numInputs ; i++){
          uncertainDataEstimator.setInputDataPointer(i, inputScalarField[i]->GetVoidPointer(0));
        }
        ...
        uncertainDataEstimator.setOutputLowerBoundField(outputLowerBoundScalarField_->GetVoidPointer(0));
        uncertainDataEstimator.setOutputUpperBoundField(outputUpperBoundScalarField_->GetVoidPointer(0));

        uncertainDataEstimator.setOutputMeanField(outputMeanField_->GetVoidPointer(0));

        uncertainDataEstimator.setBinCount(binCount_);
        for(int b=0 ; b<binCount_ ; b++){
          uncertainDataEstimator.setOutputProbability(b, outputProbabilityScalarField_[b]->GetVoidPointer(0));
        }
...
int vtkUncertainDataEstimator::FillInputPortInformation(int port, vtkInformation *info){
...
int vtkUncertainDataEstimator::FillOutputPortInformation(int port, vtkInformation *info){
...
int vtkUncertainDataEstimator::RequestData(vtkInformation *request,
  ...
  {
    stringstream msg;
    msg << "[vtkUncertainDataEstimator] Number of inputs: " << numInputs << endl;
  ...
  {
    stringstream msg;
    msg << "[vtkUncertainDataEstimator] Memory usage: " << m.getElapsedUsage()
      << " MB." << endl;
