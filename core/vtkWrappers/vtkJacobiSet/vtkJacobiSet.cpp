...
vtkStandardNewMacro(vtkJacobiSet)

vtkJacobiSet::vtkJacobiSet(){
...
template<class dataTypeU, class dataTypeV> int vtkJacobiSet::baseCall(
  vtkDataSet *input,
  vtkDataArray *uField, 
  vtkDataArray *vField){
  ...
  if((PredefinedOffset)||((UoffsetId != -1)&&(VoffsetId != -1))){
    if(OffsetFieldU.length()){
      ...
      if(offsetFieldU){
        ...
        for(int i = 0; i < offsetFieldU->GetNumberOfTuples(); i++){
    ...
    if(OffsetFieldV.length()){
      ...
      if(offsetFieldV){
        ...
        for(int i = 0; i < offsetFieldV->GetNumberOfTuples(); i++){
...
int vtkJacobiSet::doIt(vector<vtkDataSet *> &inputs, 
  ...
  {
    stringstream msg;
    msg << "[vtkJacobiSet] U-component: `" << uComponent->GetName()
      << "'" << endl;
    msg << "[vtkJacobiSet] V-component: `" << vComponent->GetName()
      << "'" << endl;
  ...
  switch(uComponent->GetDataType()){
    ...
    default:
      {
        stringstream msg;
        msg << "[vtkJacobiSet] Unsupported U-component data type :( ["
  ...
  int pointCount = 0;
  double p[3];
  for(int i = 0; i < (int) jacobiSet_.size(); i++){
  ...
  if(EdgeIds){
    vtkSmartPointer<vtkIntArray> edgeIdArray = 
    ...
    for(int i = 0; i < input->GetPointData()->GetNumberOfArrays(); i++){
      ...
      switch(scalarField->GetDataType()){
        ...
        case VTK_CHAR:
          {
            ...
            pointCount = 0;
            for(int j = 0; j < (int) jacobiSet_.size(); j++){
        ...
        default:
          {
            stringstream msg;
            msg << "[vtkJacobiSet] Scalar attachment: "
  ...
  {
    stringstream msg;
    msg << "[vtkJacobiSet] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
