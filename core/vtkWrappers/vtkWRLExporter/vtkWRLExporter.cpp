#include                  <vtkWRLExporter.h>

vtkPolyData *vtkWRLExporterPolyData_ = NULL;
...
void vtkVRMLExporter::WriteAnActor(vtkActor *anActor, FILE *fp) {

  {
    ttk::Debug debugInfo;
    stringstream msg;
    msg << "[vtkWRLExporter] Using TTK fix for VRML export..."
      << endl;
  ...
  if (inputDO->IsA("vtkCompositeDataSet")) {
      vtkCompositeDataGeometryFilter *gf = 
    ...
    if (inputDO->GetDataObjectType() != VTK_POLY_DATA) {
  ...
  // BUG fix here
  vtkWRLExporterPolyData_ = static_cast<vtkPolyData *>(pd);
  // end of BUG fix here
...
void vtkVRMLExporter::WritePointData(vtkPoints *points, vtkDataArray *normals,
  ...
  if (tcoords){
    ...
    // BUG fix here.
    if(vtkWRLExporterPolyData_){
      fprintf(fp,"          texCoordIndex[\n");
      vtkCellArray *cells = vtkWRLExporterPolyData_->GetPolys();
      vtkIdType npts = 0;
      vtkIdType *indx = NULL;
      for(cells->InitTraversal(); cells->GetNextCell(npts, indx);){
        fprintf(fp,"            ");
