#include                  <vtkIdentifiers.h>

vtkStandardNewMacro(vtkIdentifiers)

vtkIdentifiers::vtkIdentifiers(){
...
bool vtkIdentifiers::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkIdentifiers::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkIdentifiers] " << progress*100 
      << "% processed...." << endl;
...
int vtkIdentifiers::doIt(vtkDataSet *input, vtkDataSet *output){
...
#pragma omp parallel for num_threads(threadNumber_) 
  ...
  {
    stringstream msg;
    msg << "[vtkIdentifiers] Identifiers generated in "
...
int vtkIdentifiers::RequestData(vtkInformation *request, 
  ...
  {
    stringstream msg;
    msg << "[vtkIdentifiers] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
