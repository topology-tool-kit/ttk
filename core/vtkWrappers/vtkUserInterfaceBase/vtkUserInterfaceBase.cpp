...
vtkUserInterfaceBase::vtkUserInterfaceBase() {
...
int vtkUserInterfaceBase::exportScene(
...
int vtkUserInterfaceBase::init(int &argc, char **argv){
...
int vtkUserInterfaceBase::refresh(){
  ...
  if((int) surfaces_.size() != outputPortNumber){
    ...
    for(int i = 0; i < outputPortNumber; i++){
      ...
      textureMapFromFields_[i] = vtkSmartPointer<vtkTextureMapFromField>::New();
    }
  }
...
int vtkUserInterfaceBase::run(){
  ...
  {
    stringstream msg;
    msg << "[vtkUserInterfaceBase] Running user interface!" << endl;
    dMsg(cout, msg.str(), 1);
  }
...
int vtkUserInterfaceBase::switchOutput(const int &outputId){
  ...
  stringstream msg;
  msg << "[vtkUserInterfaceBase] Turning output #" << outputId 
    << " ";
...
int vtkUserInterfaceBase::switchTransparency(){

  stringstream msg;
  msg << "[vtkUserInterfaceBase] Switching transparency ";
...
int vtkUserInterfaceBase::updateScalarFieldTexture(){
