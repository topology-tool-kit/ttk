#include                  <vtkProjectionFromField.h>

vtkStandardNewMacro(vtkProjectionFromField)

vtkProjectionFromField::vtkProjectionFromField(){
...
bool vtkProjectionFromField::needsToAbort(){
  return GetAbortExecute();
}

// transmit progress status -- to copy paste in other wrappers
int vtkProjectionFromField::updateProgress(const float &progress){

  {
    stringstream msg;
    msg << "[vtkProjectionFromField] " << progress*100 
      << "% processed...." << endl;
...
int vtkProjectionFromField::doIt(vtkPointSet *input, vtkPointSet *output){
...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  {
    stringstream msg;
    msg << "[vtkProjectionFromField] Data-set projected in " 
      << t.getElapsedTime() << " s. (" 
...
int vtkProjectionFromField::RequestData(vtkInformation *request, 
  ...
  {
    stringstream msg;
    msg << "[vtkProjectionFromField] Memory usage: " << m.getElapsedUsage() 
      << " MB." << endl;
