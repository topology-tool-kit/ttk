      ...
      for(int i = 0; i < numberOfInputs_; i++) {
        ...
        if(do_min) {
          for(unsigned int j = 0;
              j < all_matchings_per_type_and_cluster[c][0][idxInCluster[i]]
                    .size();
              j++) {
            matchingTuple t
        ...
        if(do_sad) {
          for(unsigned int j = 1;
              j < all_matchings_per_type_and_cluster[c][1][idxInCluster[i]]
                    .size();
              j++) {
            matchingTuple t
        ...
        if(do_max) {
          for(unsigned int j = 0;
              j < all_matchings_per_type_and_cluster[c][2][idxInCluster[i]]
                    .size();
              j++) {
            matchingTuple t
