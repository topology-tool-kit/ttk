...
ContourTree::ContourTree(
    ...
    for(int i=0; i<nVertices; i++){
        ...
        if(node->scalarValue > maxVal) maxVal = node->scalarValue;
        if(node->scalarValue < minVal) minVal = node->scalarValue;
    }
    ...
    for(int i=0; i<nEdges; i++){
        ...
        edge->scalardistance = std::abs(nodes[edge->node1Idx]->scalarValue - nodes[edge->node2Idx]->scalarValue);
    ...
    for(CTNode* node : nodes){
        if(node->edgeList.size()==0) std::cout << "wtf?\n"<<std::flush;
        if(node->edgeList.size()>1) node->type = saddleNode;
        else{
            CTEdge* edge = arcs[node->edgeList[0]];
            CTNode* neighbor = nodes[edge->node1Idx]==node?nodes[edge->node2Idx]:nodes[edge->node1Idx];
            if(neighbor->scalarValue>node->scalarValue) node->type = minNode;
            else node->type = maxNode;
        }
    }

    binary = true;
    for(CTNode* node : nodes){
        if(node->edgeList.size() > 3) binary = false;
    }
...
Tree* ContourTree::computeRootedTree(CTNode* node, CTEdge* parent, int &id){
    ...
    // set height and size to 0/1. For inner nodes this will be updated while traversing the children
    t->size = 1;
    t->height = 0;
    ...
    // compute number of children (depends on whether current node is the root or not)
    if( parent == NULL )
        t->children = std::vector<Tree*>( node->edgeList.size() );
    else t->children = std::vector<Tree*>( node->edgeList.size() - 1 );
    ...
    for( int i=0 ; i < node->edgeList.size() ; i++ ){
        ...
        CTNode* child = nodes[edge->node1Idx]==node?nodes[edge->node2Idx]:nodes[edge->node1Idx];
        ...
        if( childTree->height+1 > t->height ) t->height = childTree->height + 1;
...
BinaryTree* ContourTree::computeRootedTree_binary(CTNode* node, CTEdge* parent, int &id){
    ...
    if(parent != NULL){
            int arcRef = arcs[node->edgeList[0]]==parent ? node->edgeList[0] : arcs[node->edgeList[1]]==parent ? node->edgeList[1] : node->edgeList[2];
...
void ContourTree::computeBranches(){
    ...
    for(int i=1; i<nodes.size(); i++){
        if(nodes[minIdx]->scalarValue > nodes[i]->scalarValue) minIdx=i;
    }

    // find path to global max
    int nextIdx = arcs[nodes[minIdx]->edgeList[0]]->node1Idx==minIdx ? arcs[nodes[minIdx]->edgeList[0]]->node2Idx : arcs[nodes[minIdx]->edgeList[0]]->node1Idx;
    ...
    while(!q.empty()){
        ...
        for(int i=1; i<path.size()-1; i++){
            ...
            for(int cE : nodes[idx]->edgeList){
                ...
                int cIdx = arcs[cE]->node1Idx==idx ? arcs[cE]->node2Idx : arcs[cE]->node1Idx;
                if(cIdx == path[i-1]) continue;
                if(cIdx == path[i+1]) continue;
...
std::pair<float,std::vector<int>> ContourTree::pathToMax(int root, int parent){
    ...
    for(int cE : nodes[root]->edgeList){
        ...
        int nextIdx = arcs[cE]->node1Idx==root ? arcs[cE]->node2Idx : arcs[cE]->node1Idx;
        if(parent == nextIdx) continue;
        if(nodes[nextIdx]->scalarValue<nodes[root]->scalarValue) continue;
...
std::pair<float,std::vector<int>> ContourTree::pathToMin(int root, int parent){
...
std::pair<std::vector<CTNode*>,std::vector<CTEdge*>> ContourTree::getGraph(){
