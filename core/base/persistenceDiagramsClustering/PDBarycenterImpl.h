...
void PDBarycenter<dataType>::runMatchingAuction(dataType* total_cost, std::vector<int> sizes, KDTree<dataType>* kdt, std::vector<KDTree<dataType>*>* correspondance_kdt_map,
                                                std::vector<dataType>* min_diag_price, std::vector<std::vector<matchingTuple>>* all_matchings, bool use_kdt)
{
#ifdef TTK_ENABLE_OPENMP
    omp_set_num_threads(threadNumber_);
#pragma omp parallel for schedule(dynamic, 1)
    ...
    for(int i = 0; i < numberOfInputs_; i++) {
        Auction<dataType> auction =
            Auction<dataType>(&current_bidder_diagrams_[i], &barycenter_goods_[i], wasserstein_, geometrical_factor_, lambda_, 0.01, kdt, *correspondance_kdt_map, (*min_diag_price)[i], use_kdt);
        std::vector<matchingTuple> matchings;
...
dataType PDBarycenter<dataType>::enrichCurrentBidderDiagrams(dataType previous_min_persistence, dataType min_persistence, std::vector<dataType> initial_diagonal_prices,
                                                             std::vector<dataType> initial_off_diagonal_prices, int min_points_to_add, bool add_points_to_barycenter)
{
    ...
    for(int i = 0; i < numberOfInputs_; i++) {
        ...
        sort(idx[i].begin(), idx[i].end(), [&persistences](int& a, int& b) { return ((persistences[a] > persistences[b]) || ((persistences[a] == persistences[b]) && (a > b))); });
...
std::vector<std::vector<matchingTuple>> PDBarycenter<dataType>::executeAuctionBarycenter(std::vector<diagramTuple>& barycenter)
{
    ...
    while(!finished) {
        ...
        runMatchingAuction(&total_cost, sizes, kdt, &correspondance_kdt_map, &min_diag_price, &all_matchings, use_kdt);
...
std::vector<std::vector<matchingTuple>> PDBarycenter<dataType>::executePartialBiddingBarycenter(std::vector<diagramTuple>& barycenter)
{
    ...
    cout << "enrich with rho : " << min_persistence << endl;  //  and barycenter size : "<<barycenter_goods_[0].size()<<endl;
    min_persistence = this->enrichCurrentBidderDiagrams(2 * max_persistence, min_persistence, min_diag_price, min_price, min_points_to_add, false);
    ...
    while(!finished) {
        Timer t;
        {
            ...
            if(use_progressive_ && n_iterations > 1 && min_persistence > rho && epsilon_decreases_) {
                ...
                // cout<<"enrich with rho : "<<rho<<"  and barycenter size : "<<barycenter_goods_[0].size()<<endl;
                min_persistence = this->enrichCurrentBidderDiagrams(min_persistence, rho, min_diag_price, min_price, min_points_to_add, true);
                // cout<<"   enrich done and barycenter size : "<<barycenter_goods_[0].size()<<endl;
                if(debugLevel_ > 2)
...
template <typename dataType>
bool PDBarycenter<dataType>::isPrecisionObjectiveMet(dataType precision_objective, int mode)
{
    if(mode == 0) {  // ABSOLUTE PRECISION
        ...
        dataType average_precision = std::accumulate(precision_.begin(), precision_.end(), 0.0) / numberOfInputs_;
