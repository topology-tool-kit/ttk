...
#pragma omp parallel for num_threads(threadNumber_)
    ...
    for(ThreadId i = 0; i < threadNumber_; i++){
      for(SimplexId j = 0; j < (SimplexId) threadedVertexEdges[i].size(); j++){
        for(SimplexId k = 0; k < (SimplexId) threadedVertexEdges[i][j].size(); k++){
...
int ZeroSkeleton::buildVertexLinks(const vector<vector<SimplexId> > &vertexStars,
...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  for(SimplexId i = 0; i < (SimplexId) vertexLinks.size(); i++){
    ...
    for(SimplexId j = 0; j < (SimplexId) vertexStars[i].size(); j++){
      for(SimplexId k = 0; k < (SimplexId) cellEdges[vertexStars[i][j]].size(); k++){
...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  for(SimplexId i = 0; i < (SimplexId) vertexLinks.size(); i++){
    ...
    for(SimplexId j = 0; j < (SimplexId) vertexStars[i].size(); j++){
      for(SimplexId k = 0; k < (SimplexId) cellTriangles[vertexStars[i][j]].size(); k++){
...
int ZeroSkeleton::buildVertexStars(const SimplexId &vertexNumber, 
  ...
  if(threadNumber_ == 1){
    ...
    for(ThreadId i = 0; i < threadNumber_; i++){
      ...
      for(SimplexId j = 0; j < (SimplexId) threadedZeroSkeleton[i]->size(); j++){
  ...
  if(threadNumber_ > 1){
    ...
    for(ThreadId i = 0; i < threadNumber_; i++){
      ...
      for(SimplexId j = 0; j < (SimplexId) threadedZeroSkeleton[i]->size(); j++){
        ...
        for(SimplexId k = 0; k < (SimplexId) (*threadedZeroSkeleton[i])[j].size(); k++){
