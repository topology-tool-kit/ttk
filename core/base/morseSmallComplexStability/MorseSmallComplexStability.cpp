...
int ttk::MorseSmallComplexStability::buildOccurenceArraysFull(
  ...
  for (int i = 0 ; i < n_blocks - 1 ; i++){
    if (coordsSource[i].size() != coordsSource[i+1].size() 
      ...
      std::cout<<"destination sizes = "<<coordsDestination[i].size()<<", "<<coordsDestination[i+1].size()<<std::endl;
      std::cout<<"destination sizes = "<<coordsSource[i].size()<<", "<<coordsSource[i+1].size()<<std::endl;
