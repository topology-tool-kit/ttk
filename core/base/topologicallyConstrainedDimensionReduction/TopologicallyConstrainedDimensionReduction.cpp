...
ttk::TCDR::TCDR(bool useCUDA, bool deterministic, int seed, int numberOfComponents, int epochs, double learningRate,
...
void ttk::TCDR::printLoss(int epoch, double loss) const {
  if (epoch % std::max(1, Epochs / 10) == 0)
    printMsg("Loss at epoch " + std::to_string(epoch) + " : " + std::to_string(loss), double(epoch)/Epochs, -1, -1, debug::LineMode::REPLACE);
}
