...
void ttk::TopologicalLoss::precomputeInputPersistence() {
  if (regul_ == REGUL::TOPOAE) {
    ...
    rpd::EdgeSets4 inputCriticalAndCascade; // [0] is MST, [1] is RNG-MST, [2] is MML, [3] is strict cascade
...
template void ttk::TopologicalLoss::computeLatent0And1Persistence(rpd::EdgeSets3 &latentPD) const;

void ttk::TopologicalLoss::computeLatentCascades(rpd::EdgeSets4 &latentCriticalAndCascades) const {
#ifdef TTK_ENABLE_CGAL
  if (latentDimension == 2)
    rpd::FastRipsPersistenceDiagram2(latent_.cpu().data_ptr<float>(), latent_.size(0)).computeRips0And1Persistence(latentCriticalAndCascades, false, false);
  else {
...
torch::Tensor ttk::TopologicalLoss::diffDistances(const torch::Tensor &data, const torch::Tensor &indices) {
