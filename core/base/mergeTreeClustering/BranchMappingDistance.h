...
namespace ttk {
    ...
    class BranchMappingDistance : virtual public Debug {
        ...
        inline dataType editCost_Wasserstein1(int n1, int p1, int n2, int p2, std::vector<dataType> &nodes1, std::vector<dataType> &nodes2){
            dataType d;
            if(n1<0){
        ...
        template <class dataType>
        inline dataType editCost_Wasserstein2(int n1, int p1, int n2, int p2, std::vector<dataType> &nodes1, std::vector<dataType> &nodes2){
            dataType d;
            if(n1<0){
                dataType b1 = nodes2[n2];
                dataType d1 = nodes2[p2];
                dataType b2 = (b1+d1)*0.5;
                dataType d2 = (b1+d1)*0.5;
                d = std::sqrt(std::abs(b1-b2)*std::abs(b1-b2)+std::abs(d1-d2)*std::abs(d1-d2));
            }
            else if(n2<0){
                dataType b1 = nodes1[n1];
                dataType d1 = nodes1[p1];
        ...
        template <class dataType>
        inline dataType editCost_Shifting(int n1, int p1, int n2, int p2, std::vector<dataType> &nodes1, std::vector<dataType> &nodes2){
            dataType d;
            if(n1<0){
                dataType b1 = nodes2[n2];
                dataType d1 = nodes2[p2];
                ...
                for(size_t l=1; l<=predecessors1[i].size(); l++){
                    ...
                    // If first subtree has more than one branch, try all decompositions
                        ...
                        for(auto child1_mb : topo1[curr1]){
                            dataType c_ = memT[child1_mb+(l+1)*dim2+nn2*dim3+0*dim4];
                ...
                for(size_t l=1; l<=predecessors2[j].size(); l++){
                    ...
                    // If first subtree has more than one branch, try all decompositions
                        ...
                        for(auto child2_mb : topo2[curr2]){
                            dataType c_ = memT[nn1+0*dim2+child2_mb*dim3+(l+1)*dim4];
                                ...
                                if(topo1[curr1].size()==2 && topo2[curr2].size()==2){
                                    ...
                                    for(auto child1_mb : topo1[curr1]){
                                        ...
                                        for(auto child2_mb : topo2[curr2]){
                                            ...
                                            dataType d_ = memT[child1_mb+(l1+1)*dim2+child2_mb*dim3+(l2+1)*dim4];
