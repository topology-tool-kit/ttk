...
namespace ttk::gph {
  ...
  private:
    ...
    void computeFacetUrquhart(UnionFind &UF) {
      ...
      TTK_PSORT(nThreads_, hyperUrquhart.begin(), hyperUrquhart.end(), [](const FiltratedQuadFacet &f1, const FiltratedQuadFacet &f2) {
        if (f1.d == f2.d)
          return f1.a > f2.a;
        return f1.d > f2.d;
      });
    ...
    void computePolygons(MultidimensionalDiagram &ph) {
      ...
      TTK_PSORT(nThreads_, urquhart.begin(), urquhart.end(), [](const FiltratedEdge &a, const FiltratedEdge &b) {
        return a.d < b.d;
      });
      UnionFind UF_p(N_p);
      ph[0].reserve(N_p-1);
    ...
    void compute1PH(MultidimensionalDiagram &ph) {
      ...
      TTK_PSORT(nThreads_, polys.begin(), polys.end(), [&](const int x1, const int x2) {
        return msa[x1].d < msa[x2].d;
      });

      std::vector<int> criticalIndices(critical1.size());
      std::iota(criticalIndices.begin(), criticalIndices.end(), 0);
      TTK_PSORT(nThreads_, criticalIndices.begin(), criticalIndices.end(), [&](const int e1_id, const int e2_id) {
        return critical1[e1_id].d < critical1[e2_id].d;
      });
      std::vector<int> criticalOrder(critical1.size());
      for (unsigned i=0; i<criticalIndices.size(); ++i)
