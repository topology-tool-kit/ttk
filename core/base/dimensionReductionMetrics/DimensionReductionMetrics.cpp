...
void ttk::DimensionReductionMetrics::execute(
  ...
  printMsg("#pts: " + std::to_string(N) + ", highDim: "
           + std::to_string(dimHigh) + ", lowDim: " + std::to_string(dimLow));
...
void ttk::DimensionReductionMetrics::computeRankBasedMetrics() {
  ...
  const int normalizingTC
    = (NeighborhoodSize < N / 2)
        ? N * NeighborhoodSize * (2 * N - 3 * NeighborhoodSize - 1)
        : N * (N - NeighborhoodSize) * (N - NeighborhoodSize - 1);
  ...
  m_lcmc = (double(LCMCSum) / (N * NeighborhoodSize)
            - double(NeighborhoodSize) / (N - 1))
           / (1 - double(NeighborhoodSize) / (N - 1));
