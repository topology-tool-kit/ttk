...
ttk::VectorSimplification::VectorSimplification() {
  this->setDebugMsgPrefix("VectorSimplification");
}

void ttk::VectorSimplification::displayStats(
    ...
    {" #Min-saddle pairs",
     std::to_string(
       std::count_if(pairs.begin(), pairs.end(),
                     [](const CandidatePair &a) { return a.type == 0; }))},
    {" #Saddle-saddle pairs",
     std::to_string(dim == 3 ? std::count_if(
                      pairs.begin(), pairs.end(),
                      [](const CandidatePair &a) { return a.type == 1; })
                             : 0)},
    {" #Saddle-max pairs",
     std::to_string(std::count_if(
       pairs.begin(), pairs.end(),
       [dim](const CandidatePair &a) { return a.type == dim - 1; }))},
  };
