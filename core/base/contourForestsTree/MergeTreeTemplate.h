...
SimplexId MergeTree::localSimplify(const SimplexId &posSeed0, const SimplexId &posSeed1)
{
   ...
   if (DEBUG) {
      std::cout << "pairs : ( threshold : " << params_->simplifyThreshold << " )" << std::endl;
      for (const auto &p : pairs) {
         ...
         std::cout << thisOriginVert << " - " << thisEndVert << " : " << thisPersist;
...
template <typename scalarType>
SimplexId MergeTree::globalSimplify(const SimplexId posSeed0, const SimplexId posSeed1)
{
   // if null threshold, leave
   if (!params_->simplifyThreshold) {
      return 0;
...
//{
   ...
   auto pairComp = [](const std::tuple<SimplexId, SimplexId, scalarType, bool> &a,
    ...
    const std::vector<std::tuple<SimplexId, SimplexId, scalarType, bool>> &sortedPairs)
{
   ...
   std::vector<std::pair<idSuperArc, idSuperArc>> valenceOffset(nbNode, std::make_pair(0, 0));
   SimplexId                                      nbPairMerged = 0;

   const bool DEBUG = false;

   if (DEBUG) {
      std::cout << "Imapct simplify on tree btwn : " << posSeed0 << " and " << posSeed1
                << std::endl;
   }
   ...
   if (DEBUG) {
      ...
      for (idNode nid = 0; nid < nbNode; nid++) {
         if (subtreeUF[nid]) {
            std::cout << "node " << getNode(nid)->getVertexId() << " is in subtree rooted :";
      ...
      if (std::get<2>(pp) < params_->simplifyThreshold) {
         ...
         if (subtreeUF[thisOriginId]->find()->getData() < 0) {
            ...
            const bool overlapB =
                ...
                scalars_->mirrorVertices[getNode(std::get<0>(receptArc))->getVertexId()] < posSeed0;
            const bool overlapA =

                scalars_->mirrorVertices[getNode(std::get<1>(receptArc))->getVertexId()] >=
                posSeed1;
            const idSuperArc na = makeSuperArc(std::get<0>(receptArc), std::get<1>(receptArc),
   ...
   for (idSuperArc arc = 0; arc < nbArcs; arc++) {
      if (getSuperArc(arc)->isMerged()) {
         ...
         if (DEBUG) {
            std::cout << " parallel merge in " << printArc(receptacleArcId) << std::endl;
            std::cout << " arc " << printArc(arc) << " size : " << getSuperArc(arc)->getVertSize()
                      << std::endl;
         }
         ...
         if (subtreeUF[downNode] && subtreeUF[upNode] &&
            ...
            if (DEBUG) {
               std::cout << "Arc between 2 degenerate with mergin " << printArc(arc) << std::endl;
               std::cout << "below recept : " << printArc(subtreeUF[downNode]->find()->getData());
               std::cout << std::endl;
               std::cout << "Above recept : " << printArc(subtreeUF[upNode]->find()->getData())
                         << std::endl;
               std::cout << std::endl;
            }
    ...
    std::vector<std::tuple<SimplexId, SimplexId, scalarType, bool>> &pairs)
{
   ...
   auto pair_sort = [](const std::tuple<SimplexId, SimplexId, scalarType, bool> &a,
                       const std::tuple<SimplexId, SimplexId, scalarType, bool> &b) {
      return std::get<2>(a) < std::get<2>(b);
   };
...
template <typename scalarType>
void MergeTree::addPair(std::vector<std::tuple<SimplexId, SimplexId, scalarType, bool>> &pairs,
                        const SimplexId &orig, const SimplexId &term, const bool goUp)
{
...
template <typename scalarType>
void MergeTree::addPair(std::vector<std::tuple<SimplexId, SimplexId, scalarType>> &pairs,
