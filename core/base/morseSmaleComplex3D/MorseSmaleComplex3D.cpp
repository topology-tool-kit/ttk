...
void MorseSmaleComplex3D::flattenSeparatricesVectors(
  ...
  const {
    ...
    for(size_t i = 1; i < separatrices.size(); ++i) {
      ...
      const auto offset = separatricesGeometry[0].size();
#ifdef TTK_ENABLE_OPENMP
#pragma omp parallel for num_threads(threadNumber_)
#endif // TTK_ENABLE_OPENMP
      ...
      separatrices[0].insert(separatrices[0].end(),
                              std::make_move_iterator(separatrices[i].begin()),
                              std::make_move_iterator(separatrices[i].end()));
      separatricesGeometry[0].insert(
        separatricesGeometry[0].end(),
        std::make_move_iterator(separatricesGeometry[i].begin()),
        std::make_move_iterator(separatricesGeometry[i].end()));
    }
}
