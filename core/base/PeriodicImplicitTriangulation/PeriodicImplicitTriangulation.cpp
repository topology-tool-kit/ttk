...
int PeriodicImplicitTriangulation::setInputGrid(const float &xOrigin, const float &yOrigin, const float &zOrigin,
  ...
  if(dimensionality_==3){
    ...
    // EdgeShift
    eshift_[0]=xDim; eshift_[1]=xDim*yDim;
    eshift_[2]=xDim; eshift_[3]=xDim*yDim;
    eshift_[4]=xDim; eshift_[5]=xDim*yDim;
    eshift_[6]=xDim; eshift_[7]=xDim*yDim;
    eshift_[8]=xDim; eshift_[9]=xDim*yDim;
    eshift_[10]=xDim; eshift_[11]=xDim*yDim;
    eshift_[12]=xDim; eshift_[13]=xDim*yDim;
    ...
    // TriangleShift
    tshift_[0]=xDim*2; tshift_[1]=xDim*yDim*2;
    tshift_[2]=xDim*2; tshift_[3]=xDim*yDim*2;
    tshift_[4]=xDim*2; tshift_[5]=xDim*yDim*2;
    tshift_[6]=xDim*2; tshift_[7]=xDim*yDim*2;
    tshift_[8]=xDim*2; tshift_[9]=xDim*yDim*2;
    tshift_[10]=xDim*2; tshift_[11]=xDim*yDim*2;
    // TetrahedronShift
    tetshift_[0]=xDim*6; tetshift_[1]=xDim*yDim*6;
...
int PeriodicImplicitTriangulation::checkAcceleration(){
  ...
  if(isAccelerated_){
    stringstream msg;
    msg << "[PeriodicImplicitTriangulation] The getVertex*() requests are accelerated."<<endl;
...
bool PeriodicImplicitTriangulation::isVertexOnBoundary(const SimplexId &vertexId) const{
...
bool PeriodicImplicitTriangulation::isEdgeOnBoundary(const SimplexId &edgeId) const {
#ifndef TTK_ENABLE_KAMIKAZE
...
bool PeriodicImplicitTriangulation::isTriangleOnBoundary(const SimplexId &triangleId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
inline SimplexId PeriodicImplicitTriangulation::getVertexNeighborNumber(const SimplexId &vertexId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(vertexId<0 or vertexId>=vertexNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getVertexNeighbor(const SimplexId &vertexId, const int &localNeighborId, SimplexId &neighborId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  ...
  if(dimensionality_==3){
    ...
    if(vertexId>0 and vertexId<nbvoxels_[Di_]){
      if(localNeighborId==0) neighborId=vertexId+1;
       ...
       if(localNeighborId==0) neighborId=vertexId+1;//a
       else neighborId=nbvoxels_[Di_];
    }
    else {
       if(localNeighborId==0) neighborId=0;//a
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getVertexNeighbors(){
...
SimplexId PeriodicImplicitTriangulation::getVertexEdgeNumber(const SimplexId &vertexId) const{
  return getVertexNeighborNumber(vertexId);
}

int PeriodicImplicitTriangulation::getVertexEdge(const SimplexId &vertexId, const int &localEdgeId, SimplexId &edgeId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  ...
  if(dimensionality_==3){
    ...
    if(vertexId>0 and vertexId<nbvoxels_[Di_]){
      ...
      if(localEdgeId==0) edgeId=vertexId;//a
      else edgeId=0;
    }
    else {
      if(localEdgeId==0) edgeId=0; 
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getVertexEdges(){
...
inline SimplexId PeriodicImplicitTriangulation::getVertexTriangleNumber(const SimplexId &vertexId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(vertexId<0 or vertexId>=vertexNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getVertexTriangle(const SimplexId &vertexId, const int &localTriangleId, SimplexId &triangleId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getVertexTriangles(){
...
SimplexId PeriodicImplicitTriangulation::getVertexLinkNumber(const SimplexId &vertexId) const{
  return getVertexStarNumber(vertexId);
}

int PeriodicImplicitTriangulation::getVertexLink(const SimplexId &vertexId, const int &localLinkId, SimplexId &linkId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getVertexLinks(){
...
inline SimplexId PeriodicImplicitTriangulation::getVertexStarNumber(const SimplexId &vertexId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(vertexId<0 or vertexId>=vertexNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getVertexStar(const SimplexId &vertexId, const int &localStarId, SimplexId &starId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getVertexStars(){
...
int PeriodicImplicitTriangulation::getVertexPoint(const SimplexId &vertexId, float& x, float& y, float& z) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(vertexId<0 or vertexId>=vertexNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getEdgeVertex(const SimplexId &edgeId, const int &localVertexId, SimplexId &vertexId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(edgeId<0 or edgeId>=edgeNumber_) return -1;
  ...
  if(dimensionality_==3){
    ...
    if(edgeId<esetshift_[0]){
      ...
      if(isAccelerated_){
        ...
        else vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1])+1+wrapXRight;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1];
        else vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1]+1+wrapXRight;
      }
    }
    //H
      ...
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1]);
        else vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1])+vshift_[0]+wrapYBottom;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1];
        else vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1]+vshift_[0]+wrapYBottom;
      }
    }
    //P
      ...
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1]);
        else vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1])+vshift_[1]+wrapZFront;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1];
        else vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1]+vshift_[1]+wrapZFront;
      }
    }
    //D1
      ...
      if (p[2] == nbvoxels_[2]) wrapZFront = -wrap_[2];
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1])+1+wrapXRight;
        else vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1])+vshift_[0]+wrapYBottom;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1]+1+wrapXRight;
        else vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1]+vshift_[0]+wrapYBottom;
      }
    }
    //D2
      ...
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1]);
        else vertexId=p[0]+(p[1]<<div_[0])+(p[2]<<div_[1])+vshift_[0]+vshift_[1]+wrapYBottom+wrapZFront;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1];
        else vertexId=p[0]+p[1]*vshift_[0]+p[2]*vshift_[1]+vshift_[0]+vshift_[1]+wrapYBottom+wrapZFront;
      }
    }
    //D3
    ...
    if(edgeId<esetshift_[0]){
      ...
      if (p[0] == nbvoxels_[Di_]) wrapXRight = -wrap_[0];
      if (p[1] == nbvoxels_[Dj_]) wrapYBottom = -wrap_[1];
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0]);
        else vertexId=p[0]+(p[1]<<div_[0])+1+wrapXRight;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0];
        else vertexId=p[0]+p[1]*vshift_[0]+1+wrapXRight;
      }
    }
    //H
    else if(edgeId<esetshift_[1]){
      ...
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0]);
        else vertexId=p[0]+(p[1]<<div_[0])+vshift_[0]+wrapYBottom;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0];
        else vertexId=p[0]+p[1]*vshift_[0]+vshift_[0]+wrapYBottom;
      }
    }
    //D1
    else if(edgeId<esetshift_[2]){
      ...
      if (p[1] == nbvoxels_[Dj_]) wrapYBottom = -wrap_[1];
      if(isAccelerated_){
        if(localVertexId==0) vertexId=p[0]+(p[1]<<div_[0])+1+wrapXRight;
        else vertexId=p[0]+(p[1]<<div_[0])+vshift_[0]+wrapYBottom;
      }
      else{
        if(localVertexId==0) vertexId=p[0]+p[1]*vshift_[0]+1+wrapXRight;
        else vertexId=p[0]+p[1]*vshift_[0]+vshift_[0]+wrapYBottom;
      }
    }
  }
...
const vector<pair<SimplexId,SimplexId>>* PeriodicImplicitTriangulation::getEdges(){
  if(!edgeList_.size()){
    Timer t;
...
inline SimplexId PeriodicImplicitTriangulation::getEdgeTriangleNumber(const SimplexId &edgeId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(edgeId<0 or edgeId>=edgeNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getEdgeTriangle(const SimplexId &edgeId, const int &localTriangleId, SimplexId &triangleId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getEdgeTriangles(){
...
inline SimplexId PeriodicImplicitTriangulation::getEdgeLinkNumber(const SimplexId &edgeId) const{
  return getEdgeStarNumber(edgeId);
}

int PeriodicImplicitTriangulation::getEdgeLink(const SimplexId &edgeId, const int &localLinkId, SimplexId &linkId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
inline SimplexId PeriodicImplicitTriangulation::getEdgeStarNumber(const SimplexId &edgeId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(edgeId<0 or edgeId>=edgeNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getEdgeStar(const SimplexId &edgeId, const int &localStarId, SimplexId &starId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
int PeriodicImplicitTriangulation::getTriangleVertex(const SimplexId &triangleId, const int &localVertexId, SimplexId &vertexId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(triangleId<0 or triangleId>=triangleNumber_) return -1;
  ...
  if(dimensionality_==3){
    ...
    SimplexId wrapXRight = 0;
    SimplexId wrapYBottom = 0;
    if (p[0]/2 == nbvoxels_[Di_]) wrapXRight = -wrap_[0];
    if (p[1] == nbvoxels_[Dj_]) wrapYBottom = -wrap_[1];
    if(id==0){
      switch(localVertexId){
        case 0: vertexId=p[0]/2+p[1]*vshift_[0]; break;
        case 1: vertexId=p[0]/2+p[1]*vshift_[0]+1+wrapXRight; break;
        case 2: vertexId=p[0]/2+p[1]*vshift_[0]+vshift_[0]+wrapYBottom; break;
      }
    }
    else{
      switch(localVertexId){
        case 0: vertexId=p[0]/2+p[1]*vshift_[0]+1+wrapXRight; break;
        case 1: vertexId=p[0]/2+p[1]*vshift_[0]+vshift_[0]+1+wrapXRight+wrapYBottom; break;
        case 2: vertexId=p[0]/2+p[1]*vshift_[0]+vshift_[0]+wrapYBottom; break;
      }
    }
  }
...
int PeriodicImplicitTriangulation::getTriangleEdge(const SimplexId &triangleId, const int &localEdgeId, SimplexId &edgeId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(triangleId<0 or triangleId>=triangleNumber_) return -1;
  ...
  if(dimensionality_==3){
    ...
    if(id==0){
      switch(localEdgeId){
        ...
        case 1: edgeId=esetshift_[0]+p[0]/2+p[1]*eshift_[2]; break;
        case 2: edgeId=esetshift_[1]+p[0]/2+p[1]*eshift_[4]; break;
      }
    }
    else{
      switch(localEdgeId){
        case 0: edgeId=p[0]/2+(p[1]+1)*eshift_[0]+wrapYBottom; break;
        case 1: edgeId=esetshift_[0]+(p[0]+1)/2+p[1]*eshift_[2]+wrapXRight; break;
        case 2: edgeId=esetshift_[1]+p[0]/2+p[1]*eshift_[4]; break;
      }
    }
  }
...
int PeriodicImplicitTriangulation::getTriangleEdges(vector<vector<SimplexId>>& edges) const{
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getTriangleEdges(){
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getTriangles(){
  if(!triangleList_.size()){
    ...
    {
      stringstream msg;
      msg << "[PeriodicImplicitTriangulation] Triangle list ("<<triangleNumber_<<" triangles) computed in "
        << t.getElapsedTime() << " s. (" << 1
        << " thread(s))."
        << endl;
      dMsg(cout, msg.str(), timeMsg);
...
int PeriodicImplicitTriangulation::getTriangleLink(const SimplexId &triangleId, const int &localLinkId, SimplexId &linkId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
inline SimplexId PeriodicImplicitTriangulation::getTriangleLinkNumber(const SimplexId &triangleId) const{
  return getTriangleStarNumber(triangleId);
}

const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getTriangleLinks(){
...
inline SimplexId PeriodicImplicitTriangulation::getTriangleStarNumber(const SimplexId &triangleId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(triangleId<0 or triangleId>=triangleNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getTriangleStar(const SimplexId &triangleId, const int &localStarId, SimplexId &starId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getTriangleStars(){
...
inline SimplexId PeriodicImplicitTriangulation::getTriangleNeighborNumber(const SimplexId &triangleId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(triangleId<0 or triangleId>=triangleNumber_) return -1;
#endif
...
int PeriodicImplicitTriangulation::getTriangleNeighbor(const SimplexId &triangleId,
    ...
    if(id){
      if(p[0]/2==nbvoxels_[Di_] and p[1]==nbvoxels_[Dj_]){
        switch(localNeighborId){
          ...
          case 1: neighborId=triangleId+1-wrap_[0]*2; break;
          case 2: neighborId=triangleId+tshift_[0]-1-wrap_[1]*2; break;
        }
      }
...
int PeriodicImplicitTriangulation::getTriangleNeighbors(vector<vector<SimplexId>>& neighbors){
...
int PeriodicImplicitTriangulation::getTetrahedronVertex(const SimplexId &tetId, const int &localVertexId, SimplexId &vertexId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
int PeriodicImplicitTriangulation::getTetrahedronEdge(const SimplexId &tetId, const int &localEdgeId, SimplexId &edgeId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(tetId<0 or tetId>=tetrahedronNumber_) return -1;
...
int PeriodicImplicitTriangulation::getTetrahedronEdges(vector<vector<SimplexId>>& edges) const{
...
int PeriodicImplicitTriangulation::getTetrahedronTriangle(const SimplexId &tetId, const int &localTriangleId, SimplexId &triangleId) const{
#ifndef TTK_ENABLE_KAMIKAZE
  if(tetId<0 or tetId>=tetrahedronNumber_) return -1;
...
int PeriodicImplicitTriangulation::getTetrahedronTriangles(vector<vector<SimplexId>>& triangles) const{
...
int PeriodicImplicitTriangulation::getTetrahedronNeighbor(const SimplexId &tetId, const int &localNeighborId, SimplexId &neighborId) const{
#ifndef TTK_ENABLE_KAMIKAZE
...
int PeriodicImplicitTriangulation::getTetrahedronNeighbors(vector<vector<SimplexId>>& neighbors){
...
SimplexId PeriodicImplicitTriangulation::getCellVertexNumber(const SimplexId &cellId) const{
  return dimensionality_+1;
}

int PeriodicImplicitTriangulation::getCellVertex(const SimplexId &cellId, const int &localVertexId, SimplexId &vertexId) const{
  if(dimensionality_==3)
...
SimplexId PeriodicImplicitTriangulation::getCellEdgeNumber(const SimplexId &cellId) const{
...
int PeriodicImplicitTriangulation::getCellEdge(const SimplexId &cellId, const int &localEdgeId, SimplexId &edgeId) const{
  if(dimensionality_==3)
...
int PeriodicImplicitTriangulation::getCellTriangle(const SimplexId &cellId, const int &localTriangleId, SimplexId &triangleId) const{
  if(dimensionality_==3)
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getCellTriangles(){
  if(!cellTriangleList_.size()){
    ...
    {
      stringstream msg;
      msg << "[PeriodicImplicitTriangulation] Cell triangles ("
        << cellNumber_ << " cell(s), "
...
SimplexId PeriodicImplicitTriangulation::getCellNeighborNumber(const SimplexId &cellId) const{
  if(dimensionality_==3)
    ...
    stringstream msg;
    msg << "[PeriodicImplicitTriangulation] getCellNeighborNumber() in 1D:" << endl;
...
int PeriodicImplicitTriangulation::getCellNeighbor(const SimplexId &cellId, const int &localNeighborId, SimplexId &neighborId) const{
  if(dimensionality_==3)
...
const vector<vector<SimplexId>>* PeriodicImplicitTriangulation::getCellNeighbors(){
  if(!cellNeighborList_.size()){
    ...
    if(dimensionality_==3)
      ...
      stringstream msg;
      msg << "[PeriodicImplicitTriangulation] getCellNeighbors() in 1D:" << endl;
      msg << "[PeriodicImplicitTriangulation] Not implemented! TODO!" << endl;
      dMsg(cerr, msg.str(), Debug::fatalMsg);
      return nullptr;
    }
    ...
    {
      stringstream msg;
      msg << "[PeriodicImplicitTriangulation] Cell neighbors ("<<getNumberOfCells()<<" cells) computed in "
        << t.getElapsedTime() << " s. (" << 1
        << " thread(s))."
        << endl;
      dMsg(cout, msg.str(), timeMsg);
