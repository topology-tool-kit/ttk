...
template <typename triangulationType>
  ...
  const int d = target.dim_;  // dimension of the target simplex

  // -----------------------------------------------------------------------
  // Helper: given a (d)-simplex `facet`, walk backward along the gradient
  // and collect every complete V-path that arrives at `facet`.
  // We use an explicit stack to avoid recursion on large meshes.
  //
  // Stack entry: (partial path so far, current frontier facet of dim d)
  // -----------------------------------------------------------------------
  using StackEntry = std::pair<VPath, Cell>;

  // Collect cofacets of a d-cell (returns cells of dimension d+1)
  auto getCofacets = [&](const Cell &localCell, std::vector<Cell> &cofacets)
  {
    cofacets.clear();
    const SimplexId nCofacets = [&]() -> SimplexId {
      switch(localCell.dim_) {
        case 0: return triangulation.getVertexEdgeNumber(localCell.id_);
        case 1: return triangulation.getEdgeTriangleNumber(localCell.id_);
        case 2: return triangulation.getTriangleStarNumber(localCell.id_);
        default: return 0;
      }
    }();
    ...
    for(SimplexId i = 0; i < nCofacets; ++i) {
      SimplexId cofacetId = -1;
      switch(localCell.dim_) {
        case 0: triangulation.getVertexEdge(localCell.id_, i, cofacetId);      break;
        case 1: triangulation.getEdgeTriangle(localCell.id_, i, cofacetId);    break;
        case 2: triangulation.getTriangleStar(localCell.id_, i, cofacetId);     break;
        default: break;
      }
      if(cofacetId != -1)
        cofacets.emplace_back(localCell.dim_ + 1, cofacetId);
    }
  };

  // -----------------------------------------------------------------------
  // Seed: iterate over every cofacet C of `target` (dim d+1).
  // If gradient pairs C → target, then target is reachable from C.
  // We initialise one stack entry per such cofacet.
  // -----------------------------------------------------------------------
  std::vector<Cell> targetCofacets;
  getCofacets(target, targetCofacets);

  printMsg("cell #" + std::to_string(target.id_)
    + " d=" + std::to_string(target.dim_)
    + " has " + std::to_string(targetCofacets.size())
    + " cofacet(s)");

  std::stack<StackEntry> stack;

  for(const Cell &cofacet : targetCofacets) {
    // Check whether the gradient arrow of `cofacet` points to `target`

    printMsg(" Considering cofacet #" + std::to_string(cofacet.id_)
      + "," + std::to_string(cofacet.dim_));
    Cell pairedFacet{};
    const SimplexId pairedFacetId = getPairedCell(cofacet, triangulation, true);
    printMsg("  paired edgeId: #" + std::to_string(pairedFacetId));
    if(pairedFacetId == -1)
      continue; // cofacet is unpaired (critical) — it does not yield a path to target this way

    pairedFacet.dim_ = cofacet.dim_ - 1;
    pairedFacet.id_ = pairedFacetId;

    if(pairedFacet.dim_ != target.dim_ || pairedFacet.id_ == target.id_)
      continue; // arrow points elsewhere

    printMsg("     -> start a path on triangle #"
      + std::to_string(cofacet.id_));
    // Seed a new partial path: [cofacet, target]
    // We will prepend cells as we walk backward, so store in reverse for now.
    VPath partialPath = {cofacet, target};
    stack.push({partialPath, cofacet});
  }

  // -----------------------------------------------------------------------
  // Backward traversal
  // -----------------------------------------------------------------------
  std::vector<Cell> cofacetsBuf;

  while(!stack.empty()) {
    auto [path, currentHighCell] = stack.top();
    stack.pop();

    // `currentHighCell` has dimension d+1.
    // We need all (d+1)-simplices S' such that gradient[S'] points to a
    // facet F' of S', and S' in turn is a cofacet of F' which is itself
    // a cofacet of some lower cell — i.e. we look one step further back.
    //
    // Concretely: the *source* of the arrow into currentHighCell is the
    // unique facet F such that gradient[currentHighCell] = F (already
    // stored as target or the last facet pushed).  To go *further back*
    // we need all (d+1)-simplices whose gradient arrow lands on some facet
    // that has currentHighCell as a cofacet.  This is a two-step look-up:
    //   (a) Facets of currentHighCell (dimension d)
    //   (b) For each such facet F, cofacets of F (dimension d+1) = {S'}
    //   (c) Keep S' where getPairedCell(S') == F

    // Collect facets of currentHighCell
    const SimplexId nFacets = [&]() -> SimplexId {
      switch(currentHighCell.dim_) {
        case 1: return triangulation.getEdgeStarNumber(currentHighCell.id_); // wrong — use facet API below
        default: break;
      }
      // Use dimension-appropriate facet count
      switch(currentHighCell.dim_) {
        case 1: return 2;                                                                  // edge has 2 vertices
        case 2: return 3;                                                                  // triangle has 3 edges
        case 3: return 4;                                                                  // tet has 4 triangles
        default: return 0;
      }
    }();

    auto getFacetId = [&](const Cell &highCell, SimplexId i, SimplexId &facetId) {
      facetId = -1;
      switch(highCell.dim_) {
        case 1: triangulation.getEdgeVertex(highCell.id_, i, facetId);       break;
        case 2: triangulation.getTriangleEdge(highCell.id_, i, facetId);     break;
        case 3: triangulation.getCellTriangle(highCell.id_, i, facetId);      break;
        default: break;
      }
    };
    ...
    for(SimplexId fi = 0; fi < nFacets; ++fi) {
      SimplexId facetId = -1;
      getFacetId(currentHighCell, fi, facetId);
      if(facetId == -1) continue;

      printMsg(" considering edge #"
        + std::to_string(facetId));

      Cell facetCell{d, facetId};

      // Skip the facet that the current arrow already points to
      // (that is `path.back()`, the cell we already came from)
      if(facetId == path.back().id_ || d != path.back().dim_)
        continue;

      // For each cofacet S' of facetCell (dim d+1), check if gradient[S'] == facetCell
      getCofacets(facetCell, cofacetsBuf);

      printMsg("    -> " + std::to_string(cofacetsBuf.size()) + " triangle(s)");

      for(const Cell &candidate : cofacetsBuf) {

        printMsg("    |-> considering triangle #" +
          std::to_string(candidate.id_));

        if(candidate.id_ == currentHighCell.id_) continue; // don't go back

        const SimplexId pairedCofacetId = getPairedCell(facetCell, triangulation);

        // we only continue if the considered simplex is paired with the cofacet
        // we're coming from
        if(pairedCofacetId != currentHighCell.id_) continue;

        Cell paired{};
        const SimplexId pairedId = getPairedCell(candidate, triangulation, true);
        if(pairedId == -1) continue;

        paired.id_ = pairedId;
        paired.dim_ = candidate.dim_ - 1;

        printMsg("      is paired with edge " + std::to_string(pairedId));

        if(paired.dim_ == d && paired.id_ == facetId) {
          // Found a predecessor: candidate → facetCell → currentHighCell
          VPath newPath = path;
          // Prepend: candidate then facetCell (we are walking backward)
          newPath.insert(newPath.begin(), facetCell);
          newPath.insert(newPath.begin(), candidate);

          stack.push({newPath, candidate});
          extended = true;
    ...
    if(!extended) {
      // No further predecessor: `currentHighCell` is a critical (d+1)-cell.
      // The path is complete.  Reverse so it reads source → … → target.
      VPath completePath = path;
      std::reverse(completePath.begin(), completePath.end());
      vpaths.push_back(std::move(completePath));
    }
  }
