...
FastRipsPersistenceDiagram2::FastRipsPersistenceDiagram2(const PointCloud &points) : n_(points.size()) {
...
FastRipsPersistenceDiagram2::FastRipsPersistenceDiagram2(float* data, int n) : n_(n) {
  // inherited from Debug: prefix will be printed at the beginning of every msg
  this->setDebugMsgPrefix("GeometricRipsPD");
  tm_.reStart();
...
template void FastRipsPersistenceDiagram2::computeDelaunayRips0And1Persistence(EdgeSets3 &ph, bool parallelSort);
...
void FastRipsPersistenceDiagram2::computeRips0And1Persistence(T &ph, bool parallelSort, bool parallelMML) {
  ...
  for (FiltratedQuadEdge const& e : urquhart_) {
    if(UF_p.find(e.e.first) != UF_p.find(e.e.second)) { //we know e is a EMST edge
      ...
      //check if e is RNG
      if (isLensEmpty(points_[e.e.first].first, points_[e.e.second].first, tree, e.d)) { //RNG edge
        critical.push_back(e);
        rng_.push_back(e);
        if constexpr(std::is_same_v<T, EdgeSets3> || std::is_same_v<T, EdgeSets4>)
          ph[1].emplace_back(e.e);
      }
...
template void FastRipsPersistenceDiagram2::computeRips0And1Persistence(MultidimensionalDiagram &ph, bool parallelSort, bool parallelMML);
template void FastRipsPersistenceDiagram2::computeRips0And1Persistence(EdgeSets3 &ph, bool parallelSort, bool parallelMML);
template void FastRipsPersistenceDiagram2::computeRips0And1Persistence(EdgeSets4 &ph, bool parallelSort, bool parallelMML);
...
void FastRipsPersistenceDiagram2::computeUrquhart(UnionFind& UF, std::vector<FiltratedEdge>& maxDelaunay, bool parallelSort) {
  for(Delaunay::Edge const& e : delaunay_.finite_edges()) {
    ...
    const double d2 = CGAL::squared_distance(points_[a].first, points_[b].first);
    ...
    if (!delaunay_.is_infinite(e.first) && CGAL::squared_distance(points_[a].first, p_k) < d2 && CGAL::squared_distance(points_[b].first, p_k) < d2)
      is_urquhart = false;
    else {
      const Point p_l = e_m.first->vertex(e_m.second)->point();
      if (!delaunay_.is_infinite(e_m.first) && CGAL::squared_distance(points_[a].first, p_l) < d2 && CGAL::squared_distance(points_[b].first, p_l) < d2)
        is_urquhart = false;
    }

    //maintain polygon structure
    if (is_urquhart) //UG edge
      urquhart_.push_back(FiltratedQuadEdge{{a,b}, e.first->id(), e_m.first->id(), d});
    else { //nUG edge: maintain UF
      const int poly1 = UF.find(e.first->id());
      const int poly2 = UF.find(e_m.first->id());
      maxDelaunay[UF.mergeRet(poly1, poly2)] = max(FiltratedEdge{{a,b}, d}, max(maxDelaunay[poly1], maxDelaunay[poly2]));
    }
  ...
  if (parallelSort)
    TTK_PSORT(globalThreadNumber_, urquhart_.begin(), urquhart_.end(), [](FiltratedQuadEdge e1, FiltratedQuadEdge e2){return e1.d < e2.d;})
  else
    std::sort(urquhart_.begin(), urquhart_.end(), [](FiltratedQuadEdge e1, FiltratedQuadEdge e2){return e1.d < e2.d;});
  ...
  for (auto it = critical.rbegin(); it != critical.rend(); ++it) {
    ...
    if (deathPoly_[latest1].d < deathPoly_[latest2].d) {
      ph[1].emplace_back(FiltratedSimplex{{e.e.first, e.e.second}, e.d},
                         FiltratedSimplex{{deathPoly_[latest1].e.first, deathPoly_[latest1].e.second}, deathPoly_[latest1].d});
      birthPoly_[latest1] = e.d;
      latest[UF.find(v1)] = latest2;
    }
    else {
      ph[1].emplace_back(FiltratedSimplex{{e.e.first, e.e.second}, e.d},
                         FiltratedSimplex{{deathPoly_[latest2].e.first, deathPoly_[latest2].e.second}, deathPoly_[latest2].d});
...
bool FastRipsPersistenceDiagram2::isLensEmpty(Point const& p1, Point const& p2, Tree const& tree, double const& d) {
...
bool FastRipsPersistenceDiagram2::isRightSemiLensEmpty(Point const& p1, Point const& p2, Tree const& tree) {
...
void FastRipsPersistenceDiagram2::computePolygonRipsDeath(bool parallel, UnionFind &UF, std::vector<int> const& indexPolys) {
          ...
          if (bound_min <= d && d <= bound_max && d < death.d) { // edge length interval allowing to be a candidate
            ...
            if (!lens_l.empty() && !lens_r.empty()) { // 2-edge
              ...
              if (r_expandable) { //expandable 2-edge, check if it is a diagonal of the current polygon (costly)
                if (indexPolys[UF.find(delaunay_.locate(CGAL::midpoint(p_i,p_j), repr_face[poly])->id())] == int(poly))
...
void FastRipsPersistenceDiagram2::reindexPolygons(UnionFind const& UF, std::vector<FiltratedEdge>const& maxDelaunay, std::vector<int>& indexPolys) {
...
void FastRipsPersistenceDiagram2::pComputePolygonRipsDeath(UnionFind &UF, std::vector<int> const& indexPolys) {
  const unsigned N_polys = deathPoly_.size();
  std::vector<std::vector<int>> vertices(N_polys);
...
void FastRipsPersistenceDiagram2::executePolygonPairCells(bool parallel, UnionFind &UF, std::vector<int> const& indexPolys, EdgeSets4 &ph) const {
