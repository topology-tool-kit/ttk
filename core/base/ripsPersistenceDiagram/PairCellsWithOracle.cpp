...
ttk::rpd::PairCellsWithOracle::PairCellsWithOracle(const PointCloud &points, MultidimensionalDiagram const& oracle, bool distanceMatrix, bool parallelSort) : n_(points.size()), parallelSort_(parallelSort), oracle_(oracle) {
...
void ttk::rpd::PairCellsWithOracle::callOracle(const PointCloud &points, MultidimensionalDiagram &oracle, double threshold) {
...
void ttk::rpd::PairCellsWithOracle::run() {
  ...
  initializeWithBound();
  printMsg("Initialized (#p=" + std::to_string(n_) + ", #e=" + std::to_string(nEdges_) + ")", 0, tm.getElapsedTime());
  pairCellsWithOracle();
...
void ttk::rpd::PairCellsWithOracle::pairCellsWithOracle() {
  for (const auto &[birth,death] : oracle_[1]) {
    if (death.second < inf) {
      ...
      triangles_.push_back({{death.first[0], death.first[1], death.first[2]}, death.second});
...
void ttk::rpd::PairCellsWithOracle::eliminateBoundaryWithOracle(id_t t_id, id_t e_id) {
  ...
  while (youngest_id != e_id) {
    youngest_id = *std::max_element(boundaries_[t_id].begin(), boundaries_[t_id].end(), [&](id_t i, id_t j) {
...
void ttk::rpd::PairCellsWithOracle::getGenerators(std::vector<Generator> &generators) const {
  for (unsigned i=0; i<triangles_.size(); ++i) {
    ...
    generators.emplace_back(boundary, std::make_pair(edges_[trianglesPartner_[i]].d, triangles_[i].d));
  }
}

void ttk::rpd::PairCellsWithOracle::getCascades(std::vector<Cascade> &cascades, EdgeSets3 &critical) const {
  fillRNG(critical);
