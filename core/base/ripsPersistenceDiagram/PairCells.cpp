...
PairCells::PairCells(const std::vector<CGAL::Epick::Point_2> &points, double upperBound, bool parallelSort, bool parallelMatrixConstruction) : n_(points.size()), bound_(upperBound), parallelSort_(parallelSort), parallelMatrixConstruction_(parallelMatrixConstruction) {
  // inherited from Debug: prefix will be printed at the beginning of every msg
  this->setDebugMsgPrefix("PairCells");

  for (int i = 1; i < n_; ++i) {
    for (int j = 0; j < i; ++j)
      compressedDM_.push_back(sqrt(CGAL::squared_distance(points[i], points[j])));
  }
}
#endif

PairCells::PairCells(const PointCloud &points, bool distanceMatrix, double upperBound, bool parallelSort, bool parallelMatrixConstruction) : n_(points.size()), bound_(upperBound), parallelSort_(parallelSort), parallelMatrixConstruction_(parallelMatrixConstruction) {
  // inherited from Debug: prefix will be printed at the beginning of every msg
  this->setDebugMsgPrefix("PairCells");
...
PairCells::PairCells(float* data, int n, int dim, double upperBound, bool parallelSort, bool parallelMatrixConstruction) : n_(n), bound_(upperBound), parallelSort_(parallelSort), parallelMatrixConstruction_(parallelMatrixConstruction) {
  // inherited from Debug: prefix will be printed at the beginning of every msg
  this->setDebugMsgPrefix("PairCells");
...
void PairCells::run() {
  ...
  if (bound_ == inf || bound_ < 0)
    ...
    initializeWithBound();
  printMsg("Initialized (#p=" + std::to_string(n_) + ", #e=" + std::to_string(nEdges_) + ", #t=" + std::to_string(nTriangles_) + ")", 0, tm.getElapsedTime());
  pairCells();
...
#pragma omp parallel for if(parallelMatrixConstruction_)
  ...
  for (id_t i=0; i < n_-2; ++i) {
    const unsigned index_i = i * (i*i - 3*i*(n_-1) + 3*n_*n_ - 6*n_ + 2) / 6;
    for (id_t j=i+1; j < n_-1; ++j) {
      ...
      for (id_t k=j+1; k < n_; ++k) {
        ...
        boundaries_[index_j + (k-j-1)] = {nEdges_-1 - (n_-i)*(n_-i-1)/2 + (j-i),
  ...
  for (const auto& [e, f] : edges_) {
    ...
    for (auto const& k : graph[j]) {
      if (graph[i].find(k) != graph[i].end()) { // c++20 -> std::unordered_set::contains
        triangles_.push_back({{i,j,k}, std::max(DM(i,j), std::max(DM(i,k), DM(j,k)))});
        boundaries_.push_back({edgeToIndex[{i, j}],
                              edgeToIndex[{i, k}],
                              edgeToIndex[{j, k}]});
      }
    }
...
void PairCells::executeKruskal() {
  ...
  if (parallelSort_)
    TTK_PSORT(globalThreadNumber_, edgesIndices_.begin(), edgesIndices_.end(), compEdges)
  else
...
void PairCells::symbolicPerturbation(double eps) { //could be improved (many calls when lots of collisions)
  bool generic;
  do {
    generic = true;
...
void PairCells::apparentPairs() {
  ...
  if (parallelSort_)
    TTK_PSORT(globalThreadNumber_, trianglesIndices_.begin(), trianglesIndices_.end(), compTriangles)
  else
    std::sort(trianglesIndices_.begin(), trianglesIndices_.end(), compTriangles);
...
ttk::rpd::id_t PairCells::eliminateBoundaries(id_t s) {
  ...
  while (!boundaries_[s].empty()) {
    const id_t e = *std::max_element(boundaries_[s].begin(), boundaries_[s].end(), [&](id_t i, id_t j) {
...
void PairCells::getDiagram(MultidimensionalDiagram &diagrams) const {
  ...
  for (const id_t &e : edgesIndices_) {
    if (edgesPartner_[e] == -2)
      diagrams[0].emplace_back(FiltratedSimplex{{0}, 0.}, FiltratedSimplex{{edges_[e].e.first, edges_[e].e.second}, edges_[e].d});
    else if (edgesPartner_[e] > 0 && edges_[e].d < triangles_[edgesPartner_[e]].d) {
      const FiltratedSimplex birth ({edges_[e].e.first, edges_[e].e.second}, edges_[e].d);
      const FiltratedSimplex death ({std::get<0>(triangles_[edgesPartner_[e]].t),
                                  std::get<1>(triangles_[edgesPartner_[e]].t),
                                  std::get<2>(triangles_[edgesPartner_[e]].t)}, triangles_[edgesPartner_[e]].d);
...
void PairCells::getDiagramAndGenerators(MultidimensionalDiagram &diagrams, std::vector<Generator> &generators) const {
  diagrams.resize(2);
  for (const id_t &e : edgesIndices_) {
    if (edgesPartner_[e] == -2)
      diagrams[0].emplace_back(FiltratedSimplex{{0}, 0.}, FiltratedSimplex{{edges_[e].e.first, edges_[e].e.second}, edges_[e].d});
      ...
      generators.emplace_back(boundary, std::make_pair(edges_[e].d, triangles_[edgesPartner_[e]].d));
    }
  }
  diagrams[0].emplace_back(FiltratedSimplex{{0}, 0.}, FiltratedSimplex{{-1}, inf});
}

void PairCells::getCascades(std::vector<Cascade> &cascades, EdgeSets3 &critical) const {
  for (const id_t &e : edgesIndices_) {
...
void PairCells::enrichCascades(std::set<Edge> &cascadeSet, EdgeSets4 &critical, std::vector<int> const& globalIndices) const {
  for (const id_t &e : edgesIndices_) {
    if (edgesPartner_[e] > 0 && edges_[e].d < triangles_[edgesPartner_[e]].d) {
      const Edge &killerEdge = edges_[cascadeEdges_[edgesPartner_[e]][0]].e;
      critical[MML].emplace_back(globalIndices[killerEdge.first], globalIndices[killerEdge.second]); //MML edge
      for (unsigned i=1; i<cascadeEdges_[edgesPartner_[e]].size()-1; ++i) {
