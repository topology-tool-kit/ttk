...
public:
  ...
  ContourAroundPoint() { this->setDebugMsgPrefix("ContourAroundPoint"); }
  ...
  int setInputField(triangulationType *triangulation, void *scalars,
...
protected:
  ...
  double compRadius() {
    if(_inpFldTriang) {
      float x, y, z;
      reinterpret_cast<triangulationType*>(_inpFldTriang)->getVertexPoint(0, x, y, z);
...
int ttk::ContourAroundPoint::setInputField(
  Triang *triangulation, void *scalars, double sizeFilter, double radius) {
...
ttk::SimplexId ttk::ContourAroundPoint::findInpFldVert(SimplexId p) const {
  ...
  const auto nv = reinterpret_cast<Triang*>(_inpFldTriang)->getNumberOfVertices();
    ...
    SimplexId vBeg, float isoval, int flag, float scalar) const
{
  ...
  auto enqueueNeighbors = [triang, &q, &handledEdges](SimplexId vSrc) {
    ...
    for(SimplexId i = 0; i < n; ++i) {
      SimplexId e; triang->getVertexEdge(vSrc, i, e);
      ...
      SimplexId vTgt; triang->getVertexNeighbor(vSrc, i, vTgt);
      q.push_back({vTgt, e});
    }
  };
    ...
    const std::set<SimplexId> &inpEdges, float isoval, int flag) const
{
  ...
  auto getOrMakeOutVert = [&](SimplexId e) {
    ...
    SimplexId p; triang->getEdgeVertex(e, 0, p);
    SimplexId q; triang->getEdgeVertex(e, 1, q);
    ...
    float px, py, pz; triang->getVertexPoint(p, px, py, pz);
    float qx, qy, qz; triang->getVertexPoint(q, qx, qy, qz);
  ...
  for(const auto e : inpEdges) {
    ...
    for(SimplexId tLoc = 0; tLoc < nTriLoc; ++tLoc) {
      SimplexId tGlo; triang->getEdgeTriangle(e, tLoc, tGlo);
      ...
      for(SimplexId eLoc = 0; eLoc < 3; ++eLoc) {
        SimplexId eGlo; triang->getTriangleEdge(tGlo, eLoc, eGlo);
