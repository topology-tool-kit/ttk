    ...
    class IcoSphere : virtual public Debug {
        ...
        public:
            ...
            */
            ...
            template <class idType> int TranslateIcoSphere(
                // Input
            ...
            */
            template <class idType> int ComputeIcoSphere(
              // Input
              const size_t& nSubdivisions,
              const float& radius,
            ...
            * Computes an icosphere for a given subdivision level, radius, and center.
            */
            template <class idType> int ComputeIcoSpheres(
              // Input
        ...
        private:
            ...
            */
            template <class idType> idType AddVertex(
                const float& x, const float& y, const float& z,
                const float& radius,
                float* vertexCoords, idType& vertexIndex
            ) const {
            ...
            */
            template <class idType> idType AddTriangle(
                const idType& i, const idType& j, const idType& k,
                idType* connectivityList, idType& triangleIndex
            ) const {
            ...
            */
            template <class idType> idType AddMidVertex(
                const idType& i, const idType& j,
                std::unordered_map<
                ...
                // Otherwise add mid vertex
                {
                    ...
                    float my = (vertexCoords[aOffset+1] + vertexCoords[bOffset+1])/2.0;
                    float mz = (vertexCoords[aOffset+2] + vertexCoords[bOffset+2])/2.0;

                    idType mIndex = this->AddVertex(mx, my, mz, radius, vertexCoords, vertexIndex);
...
template <class idType> int ttk::IcoSphere::ComputeIcoSphere(
    // Input
    const size_t& nSubdivisions,
    const float& radius,
    ...
    if(nSubdivisions>0){
        ...
        size_t nVertices = 0;
        size_t nTriangles = 0;
        this->ComputeNumberOfVerticesAndTriangles(nSubdivisions, nVertices, nTriangles);
        ...
        for(size_t s=0; s<nSubdivisions; s++){
            // swap lists
            idType* oldList = s%2==0 ? connectivityList : connectivityListTemp.data();
            idType* newList = s%2!=0 ? connectivityList : connectivityListTemp.data();
            ...
            for(size_t i=0; i<nOldTriangles; i++){
                ...
                // compute mid points
                idType a = this->AddMidVertex(
                    oldList[offset+1], oldList[offset+2],
                    processedEdges, radius, vertexCoords, vertexIndex
                );
                idType b = this->AddMidVertex(
                    oldList[offset+2], oldList[offset+3],
                    processedEdges, radius, vertexCoords, vertexIndex
                );
                idType c = this->AddMidVertex(
                    oldList[offset+3], oldList[offset+1],
                    processedEdges, radius, vertexCoords, vertexIndex
                );
    ...
    // print progress
    this->PrintMsg(
        "Computing icosphere", 1, timer.getElapsedTime(),
        ttk::debug::LineMode::REPLACE
    );
    ...
    // print stats
    {
        ...
        this->PrintMsg(
            "Complete (#"
            + std::to_string(vertexIndex) + " vertices; #"
...
template <class idType> int ttk::IcoSphere::TranslateIcoSphere(
    const size_t& icoSphereIndex,
    const size_t& nVerticesPerIcoSphere,
    const size_t& nTrianglesPerIcoSphere,
    const float* centers,
    ...
    for(size_t i=0, limit=nTrianglesPerIcoSphere*4; i<limit;){
        ...
        connectivityList[ connectivityListOffset++ ] = connectivityList[i++] + vertexIdOffset;
        connectivityList[ connectivityListOffset++ ] = connectivityList[i++] + vertexIdOffset;
        connectivityList[ connectivityListOffset++ ] = connectivityList[i++] + vertexIdOffset;
    }
...
template <class idType> int ttk::IcoSphere::ComputeIcoSpheres(
    // Input
    const size_t& nSpheres,
    const size_t& nSubdivisions,
    const float& radius,
    ...
    if( !this->ComputeNumberOfVerticesAndTriangles(nSubdivisions, nVerticesPerIcoSphere, nTrianglesPerIcoSphere) )
        return 0;

    // compute ico sphere around origin
    if( !this->ComputeIcoSphere(nSubdivisions, radius, vertexCoords, connectivityList) )
        return 0;
    ...
    for(size_t i=1; i<nSpheres; i++){
        this->TranslateIcoSphere(
            i,
            nVerticesPerIcoSphere,
            nTrianglesPerIcoSphere,
            centers,
