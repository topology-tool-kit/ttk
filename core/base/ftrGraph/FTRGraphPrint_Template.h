...
namespace ttk
{
   namespace ftr
   {
      ...
      {
         const orderedEdge oEdge = mesh_.getOrderedEdge(edgeId, localPropagation->goUp());
         std::stringstream res;
         res << "e" << edgeId << ":";
         res << "(";
      ...
      {
         std::stringstream res;
         const orderedTriangle oTriangle = mesh_.getOrderedTriangle(cellId, localPropagation->goUp());
         orderedEdge       e0, e1, e2;

         e0 = mesh_.getOrderedEdge(std::get<0>(oTriangle), localPropagation->goUp());
         e1 = mesh_.getOrderedEdge(std::get<1>(oTriangle), localPropagation->goUp());
         e2 = mesh_.getOrderedEdge(std::get<2>(oTriangle), localPropagation->goUp());
