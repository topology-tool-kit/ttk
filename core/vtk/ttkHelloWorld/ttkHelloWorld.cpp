...
int ttkHelloWorld::RequestData(vtkInformation *request,
  ...
  boundingBoxPoints->SetNumberOfPoints(
    8); // the 8 corner vertices of the bounding box

  // Get raw pointer to point coordinates
  float *boundingBoxPointCoordinates
    = (float *)boundingBoxPoints->GetVoidPointer(0);
  ...
  // Get triangulation of the input object (will create one if does not exist
  // already)
  ttk::Triangulation *triangulation = ttkAlgorithm::GetTriangulation(input);

  // Compute bounding box in the base code
  int status = this->computeBoundingBox<vtkIdType>(boundingBoxPointCoordinates,
                                                   boundingBoxConnectivityList,
                                                   triangulation, this->Scale);
  ...
  output->SetPoints(
    boundingBoxPoints); // assign the computed points to the output
