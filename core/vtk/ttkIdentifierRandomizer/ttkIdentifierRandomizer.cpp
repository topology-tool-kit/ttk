...
template <typename T>
int ttkIdentifierRandomizer::shuffleScalarFieldValuesMultiBlock(vtkMultiBlockDataSet *input, 
  ...
  for (int i = 0 ;  i < n_blocks; i++){
    ...
    const T *const inputScalarFieldPtr = static_cast<T*>(ttkUtils::GetVoidPointer(inputScalarField));
    inputValues.insert(inputValues.end(), inputScalarFieldPtr, inputScalarFieldPtr + nValues);
  }
  ...
  for (int i = 0 ; i < n_blocks; ++i){
    ...
    T *const inputArrayPtr = static_cast<T*>(ttkUtils::GetVoidPointer(inputScalarField));
    T *const outputArrayPtr = static_cast<T*>(ttkUtils::GetVoidPointer(outputArray));
...
int ttkIdentifierRandomizer::RequestData(vtkInformation *ttkNotUsed(request),
  ...
  vtkDataSet *input = vtkDataSet::SafeDownCast(vtkDataSet::GetData(inputVector[0]));
  if(input){
    ...
    this->printMsg("Shuffling " + std::string{isPointData ? "vertex" : "cell"}
      ...
      switch(outputArray->GetDataType()) {
        ...
        vtkMultiBlockDataSet *input_mb = vtkMultiBlockDataSet::SafeDownCast(vtkMultiBlockDataSet::GetData(inputVector[0]));
        vtkMultiBlockDataSet *output_mb = vtkMultiBlockDataSet::GetData(outputVector);
        ...
        int currentType=0;
        //check if the multiblock input and the input scalar field are valid and retrieve the data type
        for (int i = 0 ; i < n_blocks; i++){
