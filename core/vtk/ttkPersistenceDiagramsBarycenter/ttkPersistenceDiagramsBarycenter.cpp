...
int ttkPersistenceDiagramsBarycenter::doIt(vector<vtkDataSet *> &inputs, vector<vtkDataSet *> &outputs){
  ...
  persistenceDiagramsBarycenter_.setupTriangulation(triangulation);
  persistenceDiagramsBarycenter_.setWrapper(this);
  ...
  switch(inputScalarField->GetDataType()){
    ...
    vtkTemplateMacro(
    {
      persistenceDiagramsBarycenter_.setInputDataPointer(inputScalarField->GetVoidPointer(0));
      persistenceDiagramsBarycenter_.setOutputDataPointer(outputScalarField_->GetVoidPointer(0));
      persistenceDiagramsBarycenter_.execute<VTK_TT>(SomeIntegerArgument);
    }
    );
  }
