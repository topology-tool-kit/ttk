...
int ttkIdentifyByScalarField::RequestData(vtkInformation *request,vtkInformationVector **inputVector,vtkInformationVector *outputVector) {
  vtkDataSet *input = vtkDataSet::GetData(inputVector[0]);
  vtkDataSet *output = vtkDataSet::GetData(outputVector);

  inputScalars_ = this->GetInputArrayToProcess(0, inputVector);
  if(!inputScalars_) return 0;
  ...
  ttk::Timer t;
  this->printMsg("Computing Identifiers", 0, t.getElapsedTime(), 1, ttk::debug::LineMode::REPLACE);

  const SimplexId numberOfValues =  inputArrayAssociation==0 ? input->GetNumberOfPoints() : input->GetNumberOfCells();
  ...
  ids->SetName( inputArrayAssociation==0 ? "PointScalarFieldName" : "CellScalarFieldName" );

  SimplexId *outputIds = static_cast<SimplexId *>(ttkUtils::GetVoidPointer(ids));
