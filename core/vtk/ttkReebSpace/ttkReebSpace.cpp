...
int ttkReebSpace::doIt(vector<vtkDataSet *> &inputs,
  ...
  const vector<SimplexId> *sheet0segmentation = reebSpace_.get0sheetSegmentation();
  ...
  const vector<SimplexId> *sheet1segmentation = reebSpace_.get1sheetSegmentation();
    ...
    for(SimplexId i = 0; i < reebSpace_.getNumberOf2sheets(); i++){
      ...
      if(!sht2->pruned_){
        for(SimplexId j = 0; j < (SimplexId) sht2->triangleList_.size(); j++){
          ...
          for(SimplexId k = 0; k < (SimplexId) sht2->triangleList_[j].size(); k++){
            ...
            if(TwoSheetEdgeType){
              SimplexId polygonEdgeId = sht2->triangleList_[j][k].polygonEdgeId_;
            ...
            if(TwoSheetTetId){
              triangleTetIds->SetTuple1(triangleNumber, 
                sht2->triangleList_[j][k].tetId_);
            }
            if(TwoSheetCaseId){
              triangleCaseIds->SetTuple1(triangleNumber,
                sht2->triangleList_[j][k].caseId_);
            }
