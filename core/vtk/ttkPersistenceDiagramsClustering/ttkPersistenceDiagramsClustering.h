      ...
      public ttk::Wrapper
{
   ...
   protected:
    ...
    double getPersistenceDiagram(std::vector<diagramTuple>* diagram, vtkUnstructuredGrid* CTPersistenceDiagram_, const double spacing, const int diagramNumber);
    ...
    template <typename dataType>
      vtkSmartPointer<vtkUnstructuredGrid> createMatchings(const
          ...
          &all_CTDiagrams, const vector<vector<vector<matchingTuple>>>* matchings, double max_dimension, double spacing);
    template <typename dataType>
    ...
    template <typename dataType>
    vtkSmartPointer<vtkUnstructuredGrid> createOutputCentroids(std::vector<std::vector<diagramTuple>>* final_centroids, std::vector<int> inv_clustering, double max_dimension, double spacing);
...
template <typename dataType>
double ttkPersistenceDiagramsClustering::getPersistenceDiagram(std::vector<diagramTuple>* diagram, vtkUnstructuredGrid* CTPersistenceDiagram_, const double spacing, const int diagramNumber)
{
    vtkIntArray* vertexIdentifierScalars = vtkIntArray::SafeDownCast(CTPersistenceDiagram_->GetPointData()->GetArray(ttk::VertexScalarFieldName));

    vtkIntArray* nodeTypeScalars = vtkIntArray::SafeDownCast(CTPersistenceDiagram_->GetPointData()->GetArray("CriticalType"));

    vtkIntArray* pairIdentifierScalars = vtkIntArray::SafeDownCast(CTPersistenceDiagram_->GetCellData()->GetArray("PairIdentifier"));

    vtkIntArray* extremumIndexScalars = vtkIntArray::SafeDownCast(CTPersistenceDiagram_->GetCellData()->GetArray("PairType"));

    vtkDoubleArray* persistenceScalars = vtkDoubleArray::SafeDownCast(CTPersistenceDiagram_->GetCellData()->GetArray("Persistence"));

    vtkDoubleArray* birthScalars = vtkDoubleArray::SafeDownCast(CTPersistenceDiagram_->GetPointData()->GetArray("Birth"));
    vtkFloatArray* critCoordinates = vtkFloatArray::SafeDownCast(CTPersistenceDiagram_->GetPointData()->GetArray("Coordinates"));
    vtkDoubleArray* deathScalars = vtkDoubleArray::SafeDownCast(CTPersistenceDiagram_->GetPointData()->GetArray("Death"));
    ...
    for(int i = 0; i < pairingsSize; ++i) {
        ...
        dataType value1 = (!birthScalars) ? (dataType)x1 : (dataType)birthScalars->GetValue(2 * i);
        dataType value2 = (!deathScalars) ? (dataType)y2 : (dataType)deathScalars->GetValue(2 * i + 1);
    ...
    if(nbNonCompact > 0) {
        {
            std::stringstream msg;
            msg << "[TTKPersistenceDiagramClustering] Missed " << nbNonCompact << " pairs due to non-compactness." << std::endl;
            dMsg(std::cout, msg.str(), timeMsg);
        }
    }
...
template <typename dataType>
vtkSmartPointer<vtkUnstructuredGrid> ttkPersistenceDiagramsClustering::createOutputCentroids(std::vector<std::vector<diagramTuple>>* final_centroids, std::vector<int> inv_clustering,
                                                                                             double max_dimension, double spacing)
{
    if(debugLevel_ > 3) {
    ...
    vtkSmartPointer<vtkFloatArray> coordsScalars = vtkSmartPointer<vtkFloatArray>::New();
...
template <typename dataType>
vtkSmartPointer<vtkUnstructuredGrid> ttkPersistenceDiagramsClustering::createOutputClusteredDiagrams(std::vector<std::vector<diagramTuple>>& all_CTDiagrams, std::vector<int> inv_clustering,
                                                                                                     double max_dimension, double spacing)
{
    if(debugLevel_ > 3) {
            ...
            for(unsigned int i = 0; i < matchings_j.size(); ++i) {
               ...
               if(bidder_id<diagram->size()){
                   ...
                   if(DisplayMethod==1 && spacing > 0) {
                       // cout<<"j "<<j<<" size "<<cluster_size[inv_clustering[j]]<<endl;
                       // cout<<"count "<<count_diagram<<endl;
                       double angle = 2 * 3.1415926 * (double)(idxInCluster[j]) / cluster_size[c];
