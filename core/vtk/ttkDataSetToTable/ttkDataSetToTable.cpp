...
int ttkDataSetToTable::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkDataSetToTable::FillOutputPortInformation(int port, vtkInformation *info) {
  if(port == 0){
...
int ttkDataSetToTable::RequestData(
  ...
  std::string targetAttributeName = this->DataAssociation==0
    ? "Point"
  ...
  this->printMsg(
    "Converting "+targetAttributeName+"Data to Table",
    0,0,
