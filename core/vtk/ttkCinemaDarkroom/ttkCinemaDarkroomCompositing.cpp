...
int ttkCinemaDarkroomCompositing::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkCinemaDarkroomCompositing::RequestData(vtkInformation *request,
  ...
  ttk::Timer timer;
  this->printMsg("Compositing "+std::to_string(nInputs)+" Images",0,0,this->threadNumber_,ttk::debug::LineMode::REPLACE);
  ...
  for(size_t i = 0; i < nInputs; i++) {
    ...
    // perform compositing
    this->printMsg(
      "Compositing "+std::to_string(nInputs)+" Images",
      (float)(i+1)/(float)nInputs,timer.getElapsedTime(),this->threadNumber_,
      ttk::debug::LineMode::REPLACE
    );
  }

  this->printMsg("Compositing "+std::to_string(nInputs)+" Images",1,timer.getElapsedTime(),this->threadNumber_);
