...
int ttkCinemaDarkroomShader::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkCinemaDarkroomShader::FillOutputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
std::string ttkCinemaDarkroomShader::PerformReplacements(const std::string& source){
...
int ttkCinemaDarkroomShader::AddReplacement(const std::string& name, const std::vector<double>& values, const bool& isInt){
  auto it = this->Replacements.find(name);
  if(it == this->Replacements.end()){
    this->Replacements.emplace(
...
int ttkCinemaDarkroomShader::InitRenderer(vtkImageData* outputImage){
    ...
    if(size[0]!=dim[0] || size[1]!=dim[1]){
      ttk::Timer timer;
      this->printMsg("Initializing Renderer ("+ std::to_string(dim[0])+"x"+ std::to_string(dim[1]) +")",0,0,1,ttk::debug::LineMode::REPLACE);
      ...
      this->printMsg("Initializing Renderer ("+ std::to_string(dim[0])+"x"+ std::to_string(dim[1]) +")",1,timer.getElapsedTime(),1);
    }
...
int ttkCinemaDarkroomShader::AddTexture(vtkImageData* image, int arrayIdx, int textureIdx){
    ...
    textureObj->Create2DFromRaw(
      ...
      ttkUtils::GetVoidPointer(inputArray)
    );
...
int ttkCinemaDarkroomShader::Render(vtkImageData* image, const std::string& name){
    ttk::Timer timer;
    int dim[3]; image->GetDimensions(dim);
    this->printMsg("Rendering ("+ std::to_string(dim[0])+"x"+ std::to_string(dim[1]) +")",0,0,1,ttk::debug::LineMode::REPLACE);
    ...
    auto windowImageField = vtkUnsignedCharArray::SafeDownCast( windowImage->GetPointData()->GetAbstractArray(0) );
    windowImageField->SetName( name.data() );

    image->GetPointData()->AddArray( windowImageField );

    this->printMsg("Rendering ("+ std::to_string(dim[0])+"x"+ std::to_string(dim[1]) +")",1,timer.getElapsedTime(),1);
