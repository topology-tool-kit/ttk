...
vtkXMLWriter *getWriter(int type) {
...
int ttkCinemaWriter::ProcessDataProduct(vtkDataObject *input) {
  ...
  vtkSmartPointer<vtkXMLWriter> dataWriter;
  {
    dataWriter.TakeReference(getWriter(input->GetDataObjectType()));
    if(!dataWriter) {
      this->printErr("Unsupported data product type: "
                     + std::string(input->GetClassName()));
      return 0;
    }
    dataWriter->SetDataModeToAppended();
    dataWriter->SetCompressorTypeToZLib();
    vtkZLibDataCompressor::SafeDownCast(dataWriter->GetCompressor())
      ->SetCompressionLevel(this->CompressionLevel);
  }
  std::string productExtension
    = this->Mode == 0 ? dataWriter->GetDefaultFileExtension() : "png";
  ...
  // ===========================================================================
  {
    ...
    if(stat(csvPath.data(), &info) != 0) {
      ttk::Timer t;
      this->printMsg("Creating data.csv file", 0, ttk::debug::LineMode::NEW,
    ...
    {
      ttk::Timer t;
      this->printMsg("Reading data.csv file", 0, ttk::debug::LineMode::NEW,
                     ttk::debug::Priority::DETAIL);
    ...
    // -------------------------------------------------------------------------
    {
      ...
      if(rowsToDelete.size() > 0) {
        ...
        this->printMsg("Deleting products with same keys", 1,
                       t.getElapsedTime(), ttk::debug::LineMode::REPLACE,
                       ttk::debug::Priority::DETAIL);
      }
    }
    ...
    {
      ttk::Timer t;
      this->printMsg("Updating data.csv file", 0, ttk::debug::LineMode::NEW,
                     ttk::debug::Priority::DETAIL);
  ...
  // =========================================================================
  {
    // Write input to disk
    ttk::Timer t;
    this->printMsg("Writing data product to disk", 0, ttk::debug::LineMode::NEW,
                   ttk::debug::Priority::DETAIL);

    if(this->Mode == 0) {
      dataWriter->SetFileName(
        (this->DatabasePath + "/" + rDataProductPath).data());
      dataWriter->SetInputData(input);
      dataWriter->Write();
    } else {
...
int ttkCinemaWriter::RequestData(vtkInformation *request,
  ...
  // Print Status
  {
    std::string modeS = this->Mode == 0 ? "VTK" : "PNG";
