...
int ttkIcoSphereFromObject::FillInputPortInformation(int port, vtkInformation* info) {
    if(port==0)
...
int ttkIcoSphereFromObject::RequestData(
    ...
    if(input->IsA("vtkMultiBlockDataSet")){
        ...
        this->PrintErr("Unable to compute bounding box of "+string(input->GetClassName()));
        return 0;
    }
    ...
    float center[3] = {
        (float) (bounds[0] + dx*0.5),
        (float) (bounds[2] + dy*0.5),
    ...
    this->SetRadius(
        this->Scale * std::sqrt(dx*dx + dy*dy + dz*dz)/2.0
    );
