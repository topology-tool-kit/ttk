...
int ttkImportEmbeddingFromTable::doIt(vtkPointSet* inputDataSet,
  ...
  vtkAbstractArray* xarr=XColumn.empty()?nullptr:inputTable->GetColumnByName(XColumn.data());
  vtkAbstractArray* yarr=YColumn.empty()?nullptr:inputTable->GetColumnByName(YColumn.data());
  ...
  if(xarr->GetNumberOfTuples()!=numberOfPoints or
      yarr->GetNumberOfTuples()!=numberOfPoints or
      zarr->GetNumberOfTuples()!=numberOfPoints){
    cerr << "[ttkImportEmbeddingFromTable] Error: number of points on inputs mismatch." << endl;
    return -1;
  }
  if(xarr->GetDataType()!=yarr->GetDataType() or
      xarr->GetDataType()!=zarr->GetDataType()){
    cerr << "[ttkImportEmbeddingFromTable] Error: input columns has different data types." << endl;
    return -1;
  }
#endif
