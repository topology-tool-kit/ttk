...
int ttkForEach::RequestData(
    ...
    vtkInformation* outInfo = outputVector->GetInformationObject(0);
    double iIteration = inInfo->Get( vtkStreamingDemandDrivenPipeline::UPDATE_TIME_STEP() );
    ...
    switch (mode) {
        case 1: { // Block
            ...
            auto fd = output->GetFieldData()->GetAbstractArray( this->GetFieldDataName().data() );
            if(!fd){
                this->printErr("Field data '"+this->GetFieldDataName()+"' not found");
                return 0;
            }
    ...
    // Print status
    this->printMsg(
        "["+modes[mode]+"] Iteration: ( " + std::to_string((int)(iIteration+1)) + " / " + std::to_string((int)(iterationInformation->GetValue(1)))+" ) ",
