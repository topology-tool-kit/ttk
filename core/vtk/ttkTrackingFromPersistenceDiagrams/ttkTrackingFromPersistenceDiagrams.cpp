...
int ttkTrackingFromPersistenceDiagrams::doIt(
  ...
  int numInputs)
{
  auto outputPersistenceDiagrams = new std::vector<vtkUnstructuredGrid*>(2 * numInputs - 2);
  ...
  for (int i = 0; i < numInputs - 1; ++i)
  {
    ...
    // std::cout << "i - " << i << endl;
    // std::cout << grid1->GetFieldData()->GetArray(0)->GetTuple1(0) << endl;
    // std::cout << grid2->GetFieldData()->GetArray(0)->GetTuple1(0) << endl;
  ...
  {
    vtkUnstructuredGrid* grid = outputPersistenceDiagrams->at(i);
    if (!grid || !grid->GetCellData() ||
        !grid->GetCellData()->GetArray("Persistence"))
    {
      std::stringstream msg;
      msg << "[ttkTrackingFromPersistenceDiagrams] Inputs are not persistence diagrams." << endl;
    ...
    {
      std::stringstream msg;
      msg << "[ttkTrackingFromPersistenceDiagrams] Inputs of different data types." << endl;
      dMsg(cerr, msg.str(), fatalMsg);
      return -3;
    }
  }
  ...
  {
    vtkUnstructuredGrid* grid1 = outputPersistenceDiagrams->at(i);
    vtkUnstructuredGrid* grid2 = outputPersistenceDiagrams->at(i + 1);
    if (i % 2 == 1 && i < numPersistenceDiagramsInput - 1 &&
        grid1->GetCellData()->GetNumberOfTuples() !=
        grid2->GetCellData()->GetNumberOfTuples())
    {
      std::stringstream msg;
      msg << "[ttkTrackingFromPersistenceDiagrams] Inconsistent length or order of input diagrams." << endl;
      dMsg(cerr, msg.str(), fatalMsg);
      return -2;
    }
  }
  ...
  // TODO simulate cr whirling gaussians (for instance)
  // TODO noise robustness
  ...
  {
    auto diag1 = outputPersistenceDiagrams->at(in);
    auto diag2 = outputPersistenceDiagrams->at(in + 1);
    ...
    for (int id = 0; id < 2; ++id)
    {
      ...
      vtkUnstructuredGrid *inputDiag = outputPersistenceDiagrams->at(in + id);
      ...
      vtkDoubleArray* persistenceScalars = vtkDoubleArray::SafeDownCast(inputDiag->GetCellData()->GetArray("Persistence"));
      ...
      vtkPoints* points = (inputDiag->GetPoints());
      auto pairingsSize = (int) pairIdentifierScalars->GetNumberOfTuples();
      // auto s = (float) 0.0;

      if (!deathScalars != !birthScalars) {
        std::stringstream msg;
        msg << "[ttkTrackingFromPersistenceDiagrams] Death and birth criterion failing." << endl;
      ...
      if (pairingsSize < 1 || !vertexIdentifierScalars || !pairIdentifierScalars || !nodeTypeScalars
          || !persistenceScalars || !extremumIndexScalars || !points || !matchingIds)
      {
        std::stringstream msg;
        msg << "[ttkTrackingFromPersistenceDiagrams] Some arrays are absent." << endl;
        dMsg(std::cout, msg.str(), fatalMsg);
        return -2;
      }
      ...
      for (int i = 0; i < pairingsSize; ++i) {
        ...
        if (nbPInCell != 2) std::cout << "Bad." << endl;
    ...
    for (int i = 0; i < matchingsSize1; ++i) {
      int m1ai0 = std::get<0>(matchings1.at((unsigned long) i));
      int m1ai1 = std::get<1>(matchings1.at((unsigned long) i));

      for (int j = 0; j < matchingsSize2; ++j) {
        int m2aj0 = std::get<0>(matchings2.at((unsigned long) j));
        int m2aj1 = std::get<1>(matchings2.at((unsigned long) j));
  ...
  if (DoPostProc)
    for (unsigned int k = 0; k < trackings.size(); ++k) {
      ...
      int n1 = chainK.at(0);
      int n2 = chainK.at(chainK.size() - 1);
...
//          mergedK.insert(m);
  ...
  for (unsigned int k = 0; k < trackings.size(); ++k) {
    ...
    for (int c = 0; c < (int) chain.size() - 1; c += 2)
    {
      ...
      std::vector<matchingTuple> matchings1 = allMatchings.at((unsigned long) numStart + c);
      int d1id = numStart + c;
      int d2id = d1id + 1; // c % 2 == 0 ? d1id + 1 : d1id;
      std::vector<diagramTuple> diagram1 = allDiagrams->at((unsigned long) d1id);
      std::vector<diagramTuple> diagram2 = allDiagrams->at((unsigned long) d2id);

      // Insert segments
      vtkIdType ids[2];
      int n1 = chain.at((unsigned long) c);
      int n2 = chain.at((unsigned long) c + 1);

      // Search for right matching.
      double cost = 0.0;
      for (int m = 0; m < (int) matchings1.size(); ++m) {
        matchingTuple tup = matchings1.at(m);
        int d1id = std::get<0>(tup);
        if (d1id == n1) {
          cost = std::get<2>(tup);
          break;
        }
      ...
      if (DoPostProc) {
        ...
        if (!connected.empty()) {
          ...
          if ((numEnd2 > 0 && numStart + c > numEnd2 + 1) && min < (int) k) {
            // std::cout << "[ttkTrackingFromPersistenceDiagrams] Switched " << k << " for " << min << endl;
            cid = min;
            hasMergedFirst = numStart + c <= numEnd2 + 3;
          }

          if (hasMergedFirst) {
            std::cout << "Has merged first " << endl;

            // Replace former first end of the segment with previous ending segment.
            std::vector<BIdVertex> chain3 = std::get<2>(ttt);
            int nn = chain3.at(chain3.size() - 1);
            std::vector<diagramTuple> diagramRematch = allDiagrams->at((unsigned long) numEnd2);
  ...
  switch(outputPersistenceDiagrams->at(0)->GetCellData()->GetArray("Persistence")->GetDataType())
  {
    vtkTemplateMacro(({
      // TODO implement in basecode.
      // TODO output all persistence diagrams.
      ttk::FeatureTracking featureTracking;
      featureTracking.setWrapper(this);
      featureTracking.setNumberOfInputs(numPersistenceDiagramsInput);
      featureTracking.execute<VTK_TT>();
    }));
  }
  ...
  vtkInformationVector **inputVector, vtkInformationVector *outputVector)
{
  ...
  {
    std::stringstream msg;
    msg << "[ttkTrackingFromPersistenceDiagrams] Number of inputs: " << numInputs << endl;
