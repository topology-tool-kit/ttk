...
int ttkCinemaImaging::RequestData(vtkInformation *request,
  ...
  this->printMsg("Initializing Rendering Pipeline", 0);
...
#else
  if(this->GetGenerateScalarImages()) {
    dMsg(cout, "[ttkCinemaImaging] ERROR: Rendering scalar images requires VTK "
               "7.0 or higher\n",
         fatalMsg);
    return 0;
  }
  ...
  this->printMsg("Initializing Rendering Pipeline", 1,
                 timer.getElapsedTime() - t0, ttk::debug::LineMode::REPLACE);
