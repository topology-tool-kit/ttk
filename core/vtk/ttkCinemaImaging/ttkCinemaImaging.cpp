...
int ttkCinemaImaging::RequestData(vtkInformation *request,
  ...
  std::vector<double> defaultFocalPoint{this->FocalPoint[0],this->FocalPoint[1],this->FocalPoint[2]};
  ...
  ttkCinemaImaging::EnsureGridData( aInputGridPD, "ProjectionMode", n, {(double)this->ProjectionMode} );
  ...
  if(gridHasFocalPoint || !gridHasDirection){
    ttkCinemaImaging::EnsureGridData( aInputGridPD, "CamFocalPoint", n, defaultFocalPoint );
  ...
  // print parameters
  {
    this->printMsg({
      ...
      {"CamFocalPoint", gridHasDirection || gridHasFocalPoint ? "Grid Data" : "(" + std::to_string(defaultFocalPoint[0]) + ", "
                    + std::to_string(defaultFocalPoint[1]) + ", "
                    + std::to_string(defaultFocalPoint[2]) + ")"},
      {
          std::string(this->ProjectionMode==0 ? "CamHeight" : "CamAngle"),
          std::to_string(this->ProjectionMode==0 ? defaultHeight : defaultAngle)
      }
    });
  ...
  for(size_t b=0; b<nInputObjects; b++){
    ...
    auto inputPointSet = vtkPointSet::SafeDownCast(inputObjectAsMB->GetBlock(b));
    if(!inputPointSet->IsA("vtkUnstructuredGrid") && !inputPointSet->IsA("vtkPolyData")){
      this->printErr("Unsupported input object type.");
      return 0;
    }
    this->RequestDataSingle(
      outputImages,
...
int ttkCinemaImaging::ComputeDirFromFocalPoint(
  vtkPointSet* inputGrid
){
  auto pos = static_cast<float*>(ttkUtils::GetVoidPointer(inputGrid->GetPoints()));
  auto focal = static_cast<double*>(ttkUtils::GetVoidPointer(inputGrid->GetPointData()->GetArray("CamFocalPoint")));
...
int ttkCinemaImaging::Normalize(
    vtkDataArray* depthArray,
    const double nearFar[2]
) {
  if(!depthArray->IsA("vtkFloatArray") || depthArray->GetNumberOfComponents()!=1)
    return 0;
...
int ttkCinemaImaging::MapPointAndCellData(
  ...
  for(size_t j=0; j<nInputObjectPDArrays; j++){
    ...
    switch(outputArray->GetDataType()){
      vtkTemplateMacro(
        status = renderer->interpolateArray(
