...
int ttkPointMerger::doIt(vector<vtkDataSet *> &inputs, vector<vtkDataSet *> &outputs){
  ...
  pointMerger_.setupTriangulation(triangulation);
  pointMerger_.setWrapper(this);
  ...
  if(!outputScalarField_){
    switch(inputScalarField->GetDataType()){
      ...
      stringstream msg;
      msg << "[ttkPointMerger] Unsupported data type :(" << endl;
  ...
  switch(inputScalarField->GetDataType()){
    ...
    vtkTemplateMacro(
    {
      pointMerger_.setInputDataPointer(inputScalarField->GetVoidPointer(0));
      pointMerger_.setOutputDataPointer(outputScalarField_->GetVoidPointer(0));
      pointMerger_.execute<VTK_TT>(SomeIntegerArgument);
    }
    );
  }
