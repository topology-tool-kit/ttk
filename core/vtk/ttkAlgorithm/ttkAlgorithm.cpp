...
std::unordered_map<void*, std::pair<ttk::Triangulation*,vtkMTimeType>> ttkAlgorithm::DataSetToTriangulationMap;

ttk::Triangulation* ttkAlgorithm::GetTriangulation( vtkDataSet* dataSet ){

    this->PrintMsg(
        "Requesting triangulation for '" + std::string(dataSet->GetClassName())+"'",
    ...
    switch (dataSet->GetDataObjectType()) {
        ...
        case VTK_UNSTRUCTURED_GRID: {
            ...
            auto it = ttkAlgorithm::DataSetToTriangulationMap.find( (void*)dataSetAsUG->GetCells() );
            if(it!=ttkAlgorithm::DataSetToTriangulationMap.end()){
                if(it->second.second==dataSetAsUG->GetCells()->GetMTime()){
                    this->PrintMsg(
                        "Returning already initilized triangulation",
                        ttk::DEBUG::PRIORITY::DETAIL
                    );
                    return it->second.first;
                } else {
                    this->PrintMsg(
                        "Chached triangulation no longer valid",
                        ttk::DEBUG::PRIORITY::DETAIL
                    );
                }
            }
            ...
            if(dataSet->GetNumberOfPoints()>0){
                ...
                if(pointDataType!=VTK_FLOAT && pointDataType!=VTK_DOUBLE){
                    this->PrintErr(
                        "Unable to initialize 'ttk::Triangulation' for point precision other than 'float' or 'double'."
                    );
                    delete newTriangulation;
                    return nullptr;
                }

                newTriangulation->setInputPoints(
        ...
        case VTK_POLY_DATA: {
            ...
            int triangulationStatus = 0; // 0: no triangulation chached; 1: triangulation not valid; 2: triangulation valid
            {
                ...
                if(polyCells->GetNumberOfCells()>0){
                    ...
                    if(it!=ttkAlgorithm::DataSetToTriangulationMap.end())
                        triangulationStatus = polyCells->GetMTime()!=it->second.second ? 1 : 2;
                } else if(lineCells->GetNumberOfCells()>0) {
                    it = ttkAlgorithm::DataSetToTriangulationMap.find( (void*)lineCells );
                    if(it!=ttkAlgorithm::DataSetToTriangulationMap.end())
                        triangulationStatus = lineCells->GetMTime()!=it->second.second ? 1 : 2;
                }
            }
            ...
            if(dataSet->GetNumberOfPoints()>0){
                ...
                if(pointDataType!=VTK_FLOAT && pointDataType!=VTK_DOUBLE){
                    this->PrintErr( "Unable to initialize 'ttk::Triangulation' for point precision other than 'float' or 'double'." );
                    delete newTriangulation;
                    return 0;
                }
            ...
            // init cells
            {
                ...
                if( polyCells->GetNumberOfCells()>0 ){
                    // 2D
                    newTriangulation->setInputCells(
                        polyCells->GetNumberOfCells(),
                        polyCells->GetPointer()
                    );
                    ...
                    // 1D
                    newTriangulation->setInputCells(
                        lineCells->GetNumberOfCells(),
                        lineCells->GetPointer()
                    );
    ...
    this->PrintErr( "Unable to get/create triangulation for '" + std::string(dataSet->GetClassName())+"'" );

    return nullptr;
}

template <class vtkDataType> int prepOutput(vtkInformation* info, std::string className){
...
int ttkAlgorithm::RequestDataObject(
    ...
    for(int i=0; i < this->GetNumberOfOutputPorts(); ++i){
        ...
        if( outputPortInfo->Has(ttkAlgorithm::SAME_DATA_TYPE_AS_INPUT_PORT()) ){
            ...
            auto inPortIndex = outputPortInfo->Get(ttkAlgorithm::SAME_DATA_TYPE_AS_INPUT_PORT());
            if(inPortIndex>=this->GetNumberOfInputPorts()){
                this->PrintErr( "Input port index "+std::to_string(inPortIndex)+" specified by 'SAME_DATA_TYPE_AS_INPUT_PORT' key of output port is out of range ("+std::to_string(this->GetNumberOfInputPorts())+" input ports)." );
                return 0;
            }
            auto inInfo = inputVector[inPortIndex]->GetInformationObject(0);
            if(!inInfo){
                this->PrintErr( "No information object at port "+std::to_string(inPortIndex)+" specified by 'SAME_DATA_TYPE_AS_INPUT_PORT' key of output port." );
                return 0;
            }
            ...
            if( !output || !output->IsA(input->GetClassName()) ){
                auto newOutput = vtkSmartPointer<vtkDataObject>::Take( input->NewInstance() );
                outputPortInfo->Set( vtkDataObject::DATA_TYPE_NAME(), input->GetClassName() );
                outInfo->Set(vtkDataObject::DATA_OBJECT(), newOutput);
            }
        } else {
            // Explicitly create output by data type name
            if( !outputPortInfo->Has(vtkDataObject::DATA_TYPE_NAME()) ){
                this->PrintErr( "DATA_TYPE_NAME of output port "+std::to_string(i)+" not specified" );
                return 0;
            }
            std::string outputType = outputPortInfo->Get( vtkDataObject::DATA_TYPE_NAME() );
            ...
            if        (outputType == "vtkUnstructuredGrid"){
                ...
                this->PrintErr( "Unsupported data type for output["+std::to_string(i)+"]: "+outputType );
                return 0;
            }
        }
...
//==============================================================================
int ttkAlgorithm::ProcessRequest(
    vtkInformation* request,
    vtkInformationVector** inputVector,
    vtkInformationVector* outputVector
){
    ...
    if(request->Has(vtkCompositeDataPipeline::REQUEST_DATA_OBJECT())){
        this->PrintMsg("Processing REQUEST_DATA_OBJECT", ttk::DEBUG::PRIORITY::VERBOSE);
        return this->RequestDataObject(request, inputVector, outputVector);
    }

    // 2. Pass
    if(request->Has(vtkCompositeDataPipeline::REQUEST_INFORMATION())){
        this->PrintMsg("Processing REQUEST_INFORMATION", ttk::DEBUG::PRIORITY::VERBOSE);
        return this->RequestInformation(request, inputVector, outputVector);
    }

    // 3. Pass
    if(request->Has(vtkCompositeDataPipeline::REQUEST_UPDATE_TIME())){
        this->PrintMsg("Processing REQUEST_UPDATE_TIME", ttk::DEBUG::PRIORITY::VERBOSE);
        return this->RequestUpdateTime(request, inputVector, outputVector);
    }

    // 4. Pass
    if(request->Has(vtkCompositeDataPipeline::REQUEST_TIME_DEPENDENT_INFORMATION())){
        this->PrintMsg("Processing REQUEST_TIME_DEPENDENT_INFORMATION", ttk::DEBUG::PRIORITY::VERBOSE);
        return this->RequestUpdateTimeDependentInformation(request, inputVector, outputVector);
    }

    // 5. Pass
    if(request->Has(vtkCompositeDataPipeline::REQUEST_UPDATE_EXTENT())){
        this->PrintMsg("Processing REQUEST_UPDATE_EXTENT", ttk::DEBUG::PRIORITY::VERBOSE);
        return this->RequestUpdateExtent(request, inputVector, outputVector);
    }

    // 6. Pass
    if(request->Has(vtkCompositeDataPipeline::REQUEST_DATA_NOT_GENERATED())){
        this->PrintMsg("Processing REQUEST_DATA_NOT_GENERATED", ttk::DEBUG::PRIORITY::VERBOSE);
