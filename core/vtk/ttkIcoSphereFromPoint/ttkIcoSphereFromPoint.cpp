...
int ttkIcoSphereFromPoint::FillInputPortInformation(int port, vtkInformation* info) {
    if (port==0)
...
int ttkIcoSphereFromPoint::RequestData(
    ...
    // compute spheres
    int status = this->ttkIcoSphere::RequestData( request, inputVector, outputVector );
    if(!status)
        return 0;

    size_t nVertices = 0;
    size_t nTriangles = 0;
    this->ComputeNumberOfVerticesAndTriangles(this->GetNumberOfSubdivisions(), nVertices, nTriangles);
    ...
    for(size_t i=0, n=inputPD->GetNumberOfArrays(); i<n; i++){
        ...
        switch(newArray->GetDataType()){
            vtkTemplateMacro((copyArrayData<VTK_TT>(oldArray, newArray, nPoints, nVertices, nComponents)));
        }
