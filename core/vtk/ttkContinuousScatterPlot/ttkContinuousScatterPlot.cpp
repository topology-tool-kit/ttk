...
int ttkContinuousScatterPlot::getScalars(vtkDataSet* input){
  ...
  stringstream msg;
  msg << "[ttkContinuousScatterPlot] U-component `" 
    << inputScalars1_->GetName() << "'" << endl;
  msg << "[ttkContinuousScatterPlot] V-component `" 
...
int ttkContinuousScatterPlot::doIt(vector<vtkDataSet *> &inputs,
  ...
  vtkSmartPointer<vtkDoubleArray> densityScalars=vtkSmartPointer<vtkDoubleArray>::New();
  if(densityScalars){
  ...
  for(ttkIdType i=0; i<ScatterplotResolution[0]; i++){
    for(ttkIdType j=0; j<ScatterplotResolution[1]; j++){
      ...
      if(i<ScatterplotResolution[0]-1 and j<ScatterplotResolution[1]-1){
        ids[0]=id;
        ids[1]=id+1;
