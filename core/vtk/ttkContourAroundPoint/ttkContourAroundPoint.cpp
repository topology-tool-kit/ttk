                                ...
                                std::vector<vtkDataSet*> &outputs)
{
  ...
  memUseStream << std::fixed << std::setprecision(3) << memUseObj.getElapsedUsage() << " MB";
...
bool ttkContourAroundPoint::preprocessDomain(vtkDataSet* dataset)
{
  ...
  ttk::Triangulation* triangulation = ttkTriangulation::getTriangulation(dataset);
  if(!triangulation) {
  ...
  assert(scalars);
  const auto errorCode = _wrappedModule.setupDomain(triangulation, scalars->GetVoidPointer(0));
  if(errorCode < 0) {
  ...
  stream << "Scalar type: " << scalars->GetDataTypeAsString()
...
//  auto sizeBuf = getBuffer<float>(pData, "RegionSize", VTK_FLOAT, "float");
  ...
  const auto maxNvPerC = cells->GetMaxCellSize(); // NOTE There is no equivalent for min :-(
  if(maxNvPerC != 2) {
    vtkErrorMacro("The points must come in pairs but there is at least one cell with " +
  ...
  const double sadFac= ui_extension * 0.01; // factor for the saddle or mean of min and max
  ...
  for(vtkIdType c = 0; c < nc; ++c)
  {
    ...
    if(pIsSad || qIsSad)
    {
      ...
      _coords.push_back(point[0]); _coords.push_back(point[1]); _coords.push_back(point[2]);
...
bool ttkContourAroundPoint::postprocess()
{
  ...
  float* scalarsBuf;
  int* flagsBuf;
  _wrappedModule.getOutputField(coordsBuf, nv, cinfosBuf, nc, scalarsBuf, flagsBuf);
  if(nv == 0) // very fine area filter
    return true;

  const auto nvPerC = cinfosBuf[0];
  if(!(nvPerC == 2 || nvPerC == 3)) {
    vtkErrorMacro("Invalid number of vertices per cell: " + std::to_string(nvPerC) +
                  ". Must be 2 or 3")
     return false;
  }
  ...
  if(vtkSmartPointer<vtkPoints>::New()->GetDataType() != VTK_FLOAT) {
    vtkErrorMacro("The API has changed! We have expected the default coordinate type to be float");
    return false;
  }
  ...
  for(ttk::SimplexId c = 0, i = 0; c < nc; ++c)
  {
    ...
    if(n != nvPerC) {
      vtkErrorMacro("Output cell "+std::to_string(c)+" has "+std::to_string(n)+" vertices. " +
                    "Expected "+std::to_string(nvPerC));
      return false;
    }
    i += n + 1;
  }
  ...
  // Pass ownership of the heap-allocated raw array to the respective vtkDataArray.
...
{
  auto points = vtkSmartPointer<vtkPoints>::New();
  // longitude (x) in [0,360), latitude (y) in (-90,90) (the poles are singularites)
  ...
  auto cells = vtkSmartPointer<vtkCellArray>::New();
  // Anonymous arrays (passed directly to the function) would only be possible if
  // `InsertNextCell` were overloaded with an std::initializer_list<vtkIdType> argument :-(
  const vtkIdType c0[] = {0, 1}; cells->InsertNextCell(2, c0);
  const vtkIdType c1[] = {1, 2}; cells->InsertNextCell(2, c1);
  const vtkIdType c2[] = {2, 0}; cells->InsertNextCell(2, c2);
