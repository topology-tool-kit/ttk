...
public:
  ...
  // BEGIN set-getters macros for each parameter from the ServerManagerConfiguration XML file.
  vtkSetMacro(ui_sizeFilter, double)
  vtkGetMacro(ui_sizeFilter, double)
  
  vtkSetMacro(ui_extension, double)
  vtkGetMacro(ui_extension, double)

  vtkSetMacro(ui_scalars, std::string)
  vtkGetMacro(ui_scalars, std::string)
  // END set-getters macros

  // By default, this filter has one input and one output, of the same type.
  // Here, you can override the number of input/output ports and their respective type.
  // Make sure this is consistent with the ServerManagerConfiguration XML file and the
  // `SetNumberOfInputPorts`/`SetNumberOfOutputPorts` argument (used in the constructor).
  // The return value is interpreted as "PORT_REQUIREMENTS_FILLED" in vtkAlgorithm.cxx
  int FillInputPortInformation(int port, vtkInformation* info) override
  {
    switch(port)
    {
    case 0: info->Set(vtkDataObject::DATA_TYPE_NAME(), "vtkDataSet"); return 1;
    case 1: info->Set(vtkDataObject::DATA_TYPE_NAME(), "vtkUnstructuredGrid"); return 1;
    case 2: info->Set(vtkDataObject::DATA_TYPE_NAME(), "vtkUnstructuredGrid"); return 1;
    }
    return 0;
  }
  int FillOutputPortInformation(int port, vtkInformation* info) override
  {
    switch(port)
    {
    case 0: info->Set(vtkDataObject::DATA_TYPE_NAME(), "vtkUnstructuredGrid"); return 1;
    }
    return 0;
  }
...
protected:
  ...
  /// @return Went well?
  bool preprocessConstraints(vtkUnstructuredGrid* nodes, vtkUnstructuredGrid* arcs);
  ...
  /// For testing the general pipeline stuff, without having to execute the wrapped module.
  void makeDummyOutput();

  template<typename T>
  T* getBuffer(vtkFieldData* data, const std::string& varName, int typeCode,
               const std::string& typeName)
  {
    auto vtkArr = data->GetAbstractArray(varName.c_str());
    const std::string dataKind = dynamic_cast<vtkPointData*>(data) ? "point" : "cell";
    if(!vtkArr) {
      vtkErrorMacro("The " + dataKind + "s must have data named " + varName)
      return nullptr;
    }
    if(vtkArr->GetDataType() != typeCode) {
      vtkErrorMacro(<<"The " + dataKind +" data " + varName + " must be of type " << typeName
                    << " but it is " << vtkArr->GetDataTypeAsString())
      return nullptr;
    }
