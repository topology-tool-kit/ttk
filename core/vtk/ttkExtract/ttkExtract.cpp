...
int ttkExtract::RequestData(
    ...
    if( this->Mode==0 ){
        ...
        this->printMsg({
            {"Ext. Mode", "Geometry"},
            {"Cell Mode", std::string(cellMode==0 ? "All" : cellMode==1 ? "Any" : "Sub")},
            {"Condition", "'"+inputArrayName+"' in ["+finalExpressionString+"]"}
        });
        this->printMsg( ttk::debug::Separator::L1 );
        ...
        // ---------------------------------------------------------------------
        {
            ...
            switch( inputArray->GetDataType() ){
                ...
                //     auto contains = []( const std::vector<VTK_TT>& array, const size_t& arraySize, const VTK_TT& value ){
