...
void ContourForests::initInterfaces()
{
   ...
   for (idInterface i = 0; i < parallelParams_.nbInterfaces; ++i) {
      ...
      parallelData_.interfaces.emplace_back(scalars_->sortedVertices[partitionSize * (i + 1)]);
   }
...
void ContourForests::stitchTree(const char treetype)
{
   ...
   for (idPartition i = 0; i < parallelParams_.nbPartitions-1; i++) {
      ...
      const auto &seedPair = make_pair(parallelData_.interfaces[i].getSeed(), false);
      ...
      for (const auto &arc : curTree->treeData_.arcsCrossingAbove) {
         ...
         // the down node of the arc is not on this partition, It should have already been processed
         ...
         if (DEBUG) {
            ...
            cout << "on partition " << static_cast<unsigned>(otherPartition) << endl;
            cout << "crossing arc is now " << curTree->printArc(arc) << endl;
         }

         const idNode& curTreeStitchNodeId = curTree->getCorrespondingNodeId(stitchVertex);
         ...
         if (otherTree->isCorrespondingArc(stitchVertex)) {
            if (DEBUG){
                const idSuperArc& sa = otherTree->getCorrespondingSuperArcId(stitchVertex);
                cout << "other tree arc is : " << otherTree->printArc(sa) << endl;
            }
            const auto &arcToHide = otherTree->reverseInsertNode(curTreeStitchNode, true);
            ...
            if (DEBUG) {
                cout << "hide arc in other : " << otherTree->printArc(arcToHide) << endl;
            }
         }

         const idNode& otherTreeStitchNodeId = otherTree->getCorrespondingNodeId(stitchVertex);
         ...
         if (DEBUG) {
             ...
             cout << "other   : " << otherTree->printNode(otherTreeStitchNodeId) << endl;
         }

         // Now we can remove all arc above the stitch vertex in the current tree (noise)
         // When using debug, all arc are not hidden, they are just disconnected
         // Look at the node information, not all the arcs

         curTreeStitchNode->clearUpSuperArcs();

         // for the other tree we need to replace arc (hide the one that is replaced)
         // Exeption : the seed may contain noise directly above: we clear its down arcs if
         // it is the stitch node.
         ...
         curTree->treeData_.superArcs.emplace_back(curTreeStitchNodeId, otherTreeStitchNodeId,
                                                   false, true, i, otherPartition);
         ...
         otherTree->treeData_.superArcs.emplace_back(curTreeStitchNodeId, otherTreeStitchNodeId,
                                                     true, crossNextInterface, i, otherPartition);
         otherTreeStitchNode->addDownSuperArcId(otherTree->getNumberOfSuperArcs() - 1);
         ...
         if (DEBUG){
             cout << "arc added :" << endl;
             cout << "current : " << curTree->printArc(curTree->getNumberOfSuperArcs() - 1) << endl;
             cout << "other   : " << otherTree->printArc(otherTree->getNumberOfSuperArcs() - 1) << endl;
             cout << endl << endl;
         }
...
void ContourForests::unifyTree(const char treetype)
{
   ...
   for (idPartition partition = 0; partition < parallelParams_.nbPartitions; ++partition) {
      ...
      for (auto &l : currentTree->treeData_.leaves) {
         ...
         // if not in partition
         if (partition != 0 && isLower(leafVert, parallelData_.interfaces[partition - 1].getSeed()))
            continue;
         ...
         // this leaf have received an external arc during stitching, not a real leaf anymore
         if (curNode->getNumberOfUpSuperArcs() &&
   ...
   while (!leavesNodes.empty()) {
      ...
      if (DEBUG) {
         cout << endl;
         cout << "process : partition : " << static_cast<unsigned>(currentPartition) << endl;
      ...
      for (idSuperArc upArcPos = 0; upArcPos < nbUpArc; upArcPos++) {
         ...
         const idSuperArc & newArcId_tt = tmpTree.openSuperArc(baseNode_tt, false, false);
         ...
         do{
            ...
            if (currentNode->getNumberOfUpSuperArcs() == 1 &&
               ...
               if (DEBUG) {
                  cout << " cross : " << currentTree->printArc(currentNode->getUpSuperArcId(0));
                  cout << endl;
               }
            } else {
         ...
         const idNode &     closingNodeId    = closingTree->getCorrespondingNodeId(closingVertex);
         const unsigned &downVal = closingTree->getNode(closingNodeId)->getNumberOfDownSuperArcs();

         if (nbVisit[closingVertex] == downVal) {
            leavesNodes.emplace(closingPartition, closingNodeId);
            if (DEBUG) {
               cout << " push : partition : " << static_cast<unsigned>(closingPartition) << endl;
               cout << " push : node : " << closingTree->printNode(closingNodeId) << endl;
            }
         } else if (DEBUG){
      ...
      for (const auto &sa : parallelData_.trees[nb].getSuperArc()) {
         if (!sa.isHidden()) {
            ...
            if (sa.getDownNodeId() == nullNodes) {
               ...
               cout << parallelData_.trees[arcCTDown].getNode(sa.getDownNodeId())->getVertexId();
            }
            ...
            if (sa.getUpNodeId() == nullNodes) {
               ...
               cout << parallelData_.trees[arcCTUp].getNode(sa.getUpNodeId())->getVertexId();
            }
