...
/// \date February 2017.
///
/// \brief Command line program for Contour Forests computation.
...
int main(int argc, char **argv) {
  ...
  ttk::Debug msg;
  msg.setDebugMsgPrefix("ContourForests");

  vtkNew<ttkContourForests> cf{};
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      cf->SetInputDataObject(i, reader->GetOutput());
  ...
  // Execute the filter
  // ---------------------------------------------------------------------------
  cf->SetForceInputOffsetScalarField(forceOffset);
  cf->Update();
