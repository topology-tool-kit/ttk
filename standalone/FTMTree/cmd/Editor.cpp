...
int Editor::execute()
{
   ftmTree_->setDebugLevel(debug_);
   ftmTree_->SetdebugLevel_(debug_);
   ftmTree_->SetThreadNumber(core_);
   ftmTree_->SetUseAllCores(false);
   ...
   grid_->ShallowCopy(ftmTree_->GetOutput());
...
int Editor::saveData(const string &fileName) const
{
   ...
   writer->SetInputData((vtkUnstructuredGrid *)ftmTree_->GetOutput());
   writer->Write();
